...
# ---------------------------------------------------------------------------
def fillStructFile(param,prefix):

  structFile =""
  #define start
  structFile += "typedef struct {\n"
  for p in param:
    if p.islist:
      structFile += "\t"+p.type +" *"+ p.name+";\n"
    else:
      structFile += "\t"+p.type +" "+ p.name+";\n" 
  
  structFile+="}"+prefix+"parContent;"

  return structFile
# ---------------------------------------------------------------------------
def generate_PyParamsTemplate(params):
  PyParamsFile= ""
  PyParamsFile+="#############################################################\n###\t" 
  PyParamsFile+="Auto generate PyParams template by fillParLoader.py "
  PyParamsFile+="\t###\n#############################################################\n\n"
  #(x.name, x.type, x.islist, x.mandatory))
  PyParamsFile+= "def main():\n"

  for item in params:
    if item.islist:         
      PyParamsFile+="\t"+item.name+" = "+"[]\n"
      PyParamsFile+="\t"+"number_of_"+item.name+" = "
    else:
      PyParamsFile+="\t"+item.name+" = "
    if item.mandatory:
      PyParamsFile+="\t#mandatory parameter\n"
    else :
      PyParamsFile+="\n"
...
#-----------------------------------------------------------------------------
def replace_auto_gen_pyParams(s, auto_content):
  return s.replace("__AUTO_GEN_PARAMS__", auto_content)

#-----------------------------------------------------------------------------

def replace_auto_gen_struct(s, auto_content):
  return s.replace("__AUTO_GEN_STRUCT__", auto_content)
...
if __name__ == "__main__":
  ...
  outputPyParamsFile =  os.path.dirname(headerParContent) + "/" + tagPrefix + "pyParams.py"
  
  #outputStructFile = os.path.dirname(headerParContent) + "/" + tagPrefix + "param.h"
  #structFile = tagPrefix+"param.h_GEN"
  ...
  #paramInfoList = fillParamInfo(os.path.dirname(headerParContent))
  ...
  auto_gen_struct =  fillStructFile(paramInfoList,tagPrefix)
  auto_gen_pyparams = generate_PyParamsTemplate(paramInfoList)
  ...
  #structF = open(structFile).read()
  PyFile  = open("troll_pyParams.py_GEN").read()
  ...
  pyP = replace_auto_gen_pyParams(PyFile,auto_gen_pyparams)

  #structF  = replace_auto_gen_struct(structF,auto_gen_struct)
  ...
  # 3- Generate PyParams template from paramInfoList
  PyFile = os.path.dirname(headerParContent)+"/troll_PyParams.py"


  print("Writting result to output...")
  savefileWithContent(outputHeaderParLoader, h)
  savefileWithContent(outputSourceParLoader, s)
  savefileWithContent(outputPyParamsFile,pyP)
  #savefileWithContent(outputStructFile,structF)
