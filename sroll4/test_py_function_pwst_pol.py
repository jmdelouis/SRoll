...
def init_shape_from_files(TFLEARN,bolotab,rank,tmpid,tmpname):
    ...
    for elt in CNN_names:
      ...
      if DOFULLYCONNECTED==True:
        if elt == "MAP":
          mynetwork[elt]['fc2_weights'] = tf.constant(np.load(path_npy+"/%sC2W.npy"%(elt)))
          mynetwork[elt]['fc2_biases'] = tf.constant(np.load(path_npy+"/%sC2B.npy"%(elt)))
        else:
          mynetwork[elt]['fc2_weights'] = tf.Variable(np.load(path_npy+"/%sC2W.npy"%(elt)))
          mynetwork[elt]['fc2_biases'] = tf.Variable(np.load(path_npy+"/%sC2B.npy"%(elt)))
...
def init_network(mynetwork,signal,weights,TCO1,TSI1,hidx,idx,realpix,coef): #,in_param,in_convw,in_convb,in_flw,in_flb):
    ...
    print("**************************** PASS_INIT_NET ***************************")
    #Init calcul pwst
    #widx,widx2=init_calcul_pwst(mynetwork['MAP']['TFLEARN'],nout)
    ...
    for elt in netinfo:
      ...
      #print(elt)
      if elt=='MAP':
        params[elt] = tf.Variable(np.load(mynetwork['MAP']['TFLEARN']+'/MAPPAR.npy').reshape(mynetwork[elt]['BATCHSZ'],mynetwork[elt]['NTOTHIDDEN']))
      else:

        print(" ==> DEBUG : elt = "+str(elt)+"\n mynetwork['MAP']['TFLEARN'] = " +str(mynetwork['MAP']['TFLEARN'])+"\n mynetwork[elt]['BATCHSZ'] = "+str(mynetwork[elt]['BATCHSZ'])+"\n mynetwork[elt]['NTOTHIDDEN'] = "+str(mynetwork[elt]['NTOTHIDDEN'])+"\n")
      
        plop =np.load(mynetwork['MAP']['TFLEARN']+'/FSLPAR.npy')
        print(plop.shape)
    ...
    ampmapq=coef[netinfo["FSL"]['BATCHSZ']:netinfo["FSL"]['BATCHSZ']*2]
    ampmapu=coef[2*netinfo["FSL"]['BATCHSZ']:netinfo["FSL"]['BATCHSZ']*3]
    
    #(wu*tf_OUT_CO-wq*tf_OUT_SI)
    #(wu*tf_OUT_SI+wq*tf_OUT_CO)
    
    OUT_CO = ampmapu[bidx]*TCO1-ampmapq[bidx]*TSI1
    OUT_SI = ampmapu[bidx]*TSI1+ampmapq[bidx]*TCO1
    CO = ampmapu[bidx]*(TCO1 - iico1[hidx])-ampmapq[bidx]*(TSI1 - iisi1[hidx])
    SI = ampmapu[bidx]*(TSI1 - iisi1[hidx])+ampmapq[bidx]*(TCO1 - iico1[hidx])
