...
paramDef troll_paramDef_list[] = {
  ...
  {"AVFDUST", false, true, false},
  {"AVFCO", false, true, false},
  {"AVFSYNC", false, true, false},
  {"AVFFF", false, true, false},
  ...
  {"in_template_map_I", true, false, false},
  {"in_template_map_Q", true, false, false},
  {"in_template_map_U", true, false, false},
  ...
  {"in_polar_fit_Q", true, false, false},
  {"in_polar_fit_U", true, false, false},
  ...
  {"AVG12CO", false, false, false},
  {"AVG13CO", false, false, false},
  {"AVGDUST100", false, false, false},
  {"AVGFREEFREE", false, false, false},
  {"AVGSYNCHRO", false, false, false},
  {"saveCOV", false, false, false},
  {"saveCO", false, false, false},
  {"delta_psi", true, false, false},
  {"MAP_CNN", false, false, false},
  {"INST_CNN", false, false, false},
};
int troll_paramDef_list_size = 115;
...
int troll_updateParam(troll_parContent *param, char *name, PIOSTRING *value, PIOLONG list_size) {
  ...
  if (strcmp(name, "BeginRing") == 0) {
  ...
  else if (strcmp(name, "ADDCO13") == 0) {
    param->flag_ADDCO13 = _PAR_TRUE;
    param->n_ADDCO13 = list_size;
    param->ADDCO13 = malloc(list_size * sizeof(PIODOUBLE));
    if (param->ADDCO13 == NULL) {
      perror("Error");
      return 1;
    }
    int i;
    for (i = 0; i < list_size; i++) {
      errno = 0;
      param->ADDCO13[i] = myRead_PIODOUBLE(value[i]);
      if (errno != 0) {
        fprintf(stderr, "ERROR: 'ADDCO13': Unable to convert value '%s' to target type %s\n", value[i], "PIODOUBLE");
        return 1;
      }
    }
  }
  ...
  else if (strcmp(name, "AVFDUST") == 0) {
    errno = 0;
    param->AVFDUST = myRead_PIOINT(*value);
    if (errno != 0) {
      fprintf(stderr, "ERROR: 'AVFDUST': Unable to convert value '%s' to target type %s\n", *value, "PIOINT");
      return 1;
    }
  }
  else if (strcmp(name, "AVFCO") == 0) {
    errno = 0;
    param->AVFCO = myRead_PIOINT(*value);
    if (errno != 0) {
      fprintf(stderr, "ERROR: 'AVFCO': Unable to convert value '%s' to target type %s\n", *value, "PIOINT");
      return 1;
    }
  }
  else if (strcmp(name, "AVFSYNC") == 0) {
    errno = 0;
    param->AVFSYNC = myRead_PIOINT(*value);
    if (errno != 0) {
      fprintf(stderr, "ERROR: 'AVFSYNC': Unable to convert value '%s' to target type %s\n", *value, "PIOINT");
      return 1;
    }
  }
  else if (strcmp(name, "AVFFF") == 0) {
    errno = 0;
    param->AVFFF = myRead_PIOINT(*value);
    if (errno != 0) {
      fprintf(stderr, "ERROR: 'AVFFF': Unable to convert value '%s' to target type %s\n", *value, "PIOINT");
      return 1;
    }
  }
    ...
    param->flag_in_template_map_I = _PAR_TRUE;
    param->n_in_template_map_I = list_size;
    param->in_template_map_I = malloc(list_size * sizeof(PIOSTRING));
    if (param->in_template_map_I == NULL) {
      perror("Error");
      return 1;
    }
    int i;
    for (i = 0; i < list_size; i++) {
      strcpy(param->in_template_map_I[i], value[i]);
    }
  }
  else if (strcmp(name, "in_template_map_Q") == 0) {
    param->flag_in_template_map_Q = _PAR_TRUE;
    param->n_in_template_map_Q = list_size;
    param->in_template_map_Q = malloc(list_size * sizeof(PIOSTRING));
    if (param->in_template_map_Q == NULL) {
      perror("Error");
      return 1;
    }
    int i;
    for (i = 0; i < list_size; i++) {
      strcpy(param->in_template_map_Q[i], value[i]);
    }
  }
  else if (strcmp(name, "in_template_map_U") == 0) {
    param->flag_in_template_map_U = _PAR_TRUE;
    param->n_in_template_map_U = list_size;
    param->in_template_map_U = malloc(list_size * sizeof(PIOSTRING));
    if (param->in_template_map_U == NULL) {
      perror("Error");
      return 1;
    }
    int i;
    for (i = 0; i < list_size; i++) {
      strcpy(param->in_template_map_U[i], value[i]);
    }
  }
    ...
    param->n_in_polar_fit_Q = list_size;
    param->in_polar_fit_Q = malloc(list_size * sizeof(PIOSTRING));
    if (param->in_polar_fit_Q == NULL) {
      perror("Error");
      return 1;
    }
    int i;
    for (i = 0; i < list_size; i++) {
      strcpy(param->in_polar_fit_Q[i], value[i]);
    }
  }
  else if (strcmp(name, "in_polar_fit_U") == 0) {
    param->flag_in_polar_fit_U = _PAR_TRUE;
    param->n_in_polar_fit_U = list_size;
    param->in_polar_fit_U = malloc(list_size * sizeof(PIOSTRING));
    if (param->in_polar_fit_U == NULL) {
      perror("Error");
      return 1;
    }
    int i;
    for (i = 0; i < list_size; i++) {
      strcpy(param->in_polar_fit_U[i], value[i]);
    }
  }
  ...
  else if (strcmp(name, "AVG12CO") == 0) {
    param->flag_AVG12CO = _PAR_TRUE;
    errno = 0;
    param->AVG12CO = myRead_PIODOUBLE(*value);
    if (errno != 0) {
      fprintf(stderr, "ERROR: 'AVG12CO': Unable to convert value '%s' to target type %s\n", *value, "PIODOUBLE");
      return 1;
    }
  }
  else if (strcmp(name, "AVG13CO") == 0) {
    param->flag_AVG13CO = _PAR_TRUE;
    errno = 0;
    param->AVG13CO = myRead_PIODOUBLE(*value);
    if (errno != 0) {
      fprintf(stderr, "ERROR: 'AVG13CO': Unable to convert value '%s' to target type %s\n", *value, "PIODOUBLE");
      return 1;
    }
  }
  ...
  else if (strcmp(name, "AVGFREEFREE") == 0) {
    param->flag_AVGFREEFREE = _PAR_TRUE;
    errno = 0;
    param->AVGFREEFREE = myRead_PIODOUBLE(*value);
    if (errno != 0) {
      fprintf(stderr, "ERROR: 'AVGFREEFREE': Unable to convert value '%s' to target type %s\n", *value, "PIODOUBLE");
      return 1;
    }
  }
  else if (strcmp(name, "AVGSYNCHRO") == 0) {
    param->flag_AVGSYNCHRO = _PAR_TRUE;
    errno = 0;
    param->AVGSYNCHRO = myRead_PIODOUBLE(*value);
    if (errno != 0) {
      fprintf(stderr, "ERROR: 'AVGSYNCHRO': Unable to convert value '%s' to target type %s\n", *value, "PIODOUBLE");
      return 1;
    }
  }
  ...
  else if (strcmp(name, "saveCO") == 0) {
    param->flag_saveCO = _PAR_TRUE;
    errno = 0;
    param->saveCO = myRead_PIOINT(*value);
    if (errno != 0) {
      fprintf(stderr, "ERROR: 'saveCO': Unable to convert value '%s' to target type %s\n", *value, "PIOINT");
      return 1;
    }
  }
...
/* -------------------------------------------------------------------------------------------*/
//plip
int troll_readParam(troll_parContent *param, const char *filename) {
  ...
  //PIOSTRING currentListValues[100];
  ...
  for(int i =0;i<troll_paramDef_list_size;i++){
    ...
    if(valid_param == 0){     
      ...
      //if(mpi_rank==0)fprintf(stderr,"name = %s\n",name);
      //If list of params
      if(troll_paramDef_list[i].isList){    
        int ListSize = getListSize(pyParams,name);
        //if(mpi_rank == 0) fprintf(stderr,"ListSize = %d\n",ListSize);
	//If list not null
	if(ListSize!=-1) {
	  ...
	  while(currentListIndex!=ListSize && flag_parsing == 0){
          ...
          troll_updateParam(param,name,currentListValues,currentListIndex);
          }
      }else{
        ...
        if(pyValue != NULL){
          ...
          //if(mpi_rank==0) fprintf(stderr,"value = %s / name = %s \n",value,name);
        }
      }
    }else{
      //DEBUG
      //if(mpi_rank==0) fprintf(stderr,"Invalid params = %s\n",troll_paramDef_list[i].name);    
    }
  }    
  return 0;
}
...
/*
int troll_readParam_old(troll_parContent *param, const char *filename) {
