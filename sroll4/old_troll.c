...
void InitPython(WrapPython *mywrap, char *myfunct, int rank)
{
  ...
  mywrap->initFromFile = CALLPYTHON(mywrap->Dict, "init_shape_from_file");
   ...
   // if (flgpix[k]>0) {
	...
	for (l1=0;l1<ndata;l1++) {
	  ...
	  if (flg_rg[htmp[l1].ib][ri1]!=0) {
	    ...
	    hidx[all_ndata]=k;
	    int l_idx=htmp[l1].ib+iring*CNN_XSIZE*nbolo+ipha*nbolo;
  ...
  for (m=0;m<npixbeam;m++)  {
    if (DOCNN[m]!=0) {
      ...
      *iter_docnn = DOCNN[m]-1;
      ...
      //Test  debug
      //plop Param->Signal_noPS -- For simus
      
      const char* path_simu=Param->MAP[0];
      PyObject *str_psimu = PyString_FromString(path_simu);
      arglist = PyTuple_New(3);
      PyTuple_SetItem(arglist,0,myrun);
      PyTuple_SetItem(arglist,1,py_num);
      PyTuple_SetItem(arglist,2,str_psimu);
