...
// This is for srand48() and drand48() declaration
...
void minimize(double *ix2,double *gain)
{
    ...
    if (ndata>0&&flgpix[k]>0) {
      ...
      for (l1=0;l1<ndata;l1++) {
        ...
        if (flg_rg[htmp[l1].ib][ri1]!=0) {
          ...
          if (Param->FITTHETA&&itbogo>0) {
            dthetai[htmp[l1].ib+k*nbolo]+=htmp[l1].w*(-SI1*qmat[k]+CO1*umat[k]);
            dthetaq[htmp[l1].ib+k*nbolo]+=htmp[l1].w*(-SI1*qmat[k]+CO1*umat[k])*CO1;
            dthetau[htmp[l1].ib+k*nbolo]+=htmp[l1].w*(-SI1*qmat[k]+CO1*umat[k])*SI1;
          }
        }
      }
      ...
      for (ib=0;ib<nbolo;ib++) {
        ...
        if (Param->FITTHETA&&itbogo>0) {
          solvemap(dthetai+ib+k*nbolo,dthetaq+ib+k*nbolo,dthetau+ib+k*nbolo,II[k],IQ[k],IU[k],QQ[k],QU[k],UU[k]);
        }
      }
      for (l1=0;l1<ndata;l1++) {
        long ri1=htmp[l1].rg-globalBeginRing;
      ...
      for (l1=0;l1<ndata;l1++) {
        ...
        if (flg_rg[htmp[l1].ib][ri1]!=0) {
          ...
          if (Param->FITTHETA&&itbogo>0) {
            for (ib=0;ib<nbolo;ib++) 
              ctheta[ib]=-(dthetai[ib+k*nbolo]+CO1*dthetaq[ib+k*nbolo]+SI1*dthetau[ib+k*nbolo]);
            ctheta[htmp[l1].ib]+=(-SI1*qmat[k]+CO1*umat[k]);
          }
          long ir;
          /////////////////  OFFSET
          ...
          for (ir=0;ir<nbolo;ir++) {
            ...
            ///////////// THETA
            if (Param->FITTHETA>0&&itbogo>0) {
              b2[newnr[nbolo]+nmatdust+nmatco+npixbeam*nbolo+ir]+=ww*tmp*ctheta[ir];
              hit2[newnr[nbolo]+nmatdust+nmatco+npixbeam*nbolo+ir]+=ww*ctheta[ir]*ctheta[ir];
            }
          }
        }
      }
  ...
  if (rank==0) {
    ...
    if (Param->FITTHETA&&itbogo>0) {
      soff=0;
      for (i=0;i<nbolo;i++) 
        soff+=hit2[newnr[nbolo]+nmatdust+nmatco+npixbeam*nbolo]*
          ix2[newnr[nbolo]+nmatdust+nmatco+npixbeam*nbolo+i];
      for (i=0;i<nbolo;i++) q2[newnr[nbolo]+nmatdust+nmatco+npixbeam*nbolo+i]=soff;
    }
  }
  ...
  for (k=0;k<nnbpix;k++) if (flgpix[k]>0) {
    ...
    if (ndata>0) {
      ...
      for (ib=0;ib<nbolo;ib++) {
        ...
        if (Param->FITTHETA&&itbogo>0) {
          vali+=dthetai[ib+k*nbolo]  *ix2[newnr[nbolo]+nmatdust+nmatco+nmatdust+npixbeam*nbolo+ib];
          valq+=dthetaq[ib+k*nbolo]  *ix2[newnr[nbolo]+nmatdust+nmatco+nmatdust+npixbeam*nbolo+ib];
          valu+=dthetau[ib+k*nbolo]  *ix2[newnr[nbolo]+nmatdust+nmatco+nmatdust+npixbeam*nbolo+ib];
        } 
      }
      double qri=0;
      double qrq=0;
      ...
      for (l1=0;l1<ndata;l1++) {
        ...
        if (flg_rg[htmp[l1].ib][ri1]!=0) {
          ...
          if (Param->FITTHETA&&itbogo>0) 
            val2+=ix2[newnr[nbolo]+nmatdust+nmatco+nfreefree+npixbeam*nbolo+htmp[l1].ib]
              *(-SI1*qmat[k]+CO1*umat[k]);
          ...
          ////////// THETA
          if (Param->FITTHETA&&itbogo>0) 
            q2[newnr[nbolo]+nmatdust+nmatco+nmatdust+npixbeam*nbolo+htmp[l1].ib]+=
              ww*val2*(-SI1*qmat[k]+CO1*umat[k]);
      ...
      for (ib=0;ib<nbolo;ib++) {
        ...
        ////////// THETA
        if (Param->FITTHETA&&itbogo>0) 
          q2[newnr[nbolo]+nmatdust+nmatco+nfreefree+npixbeam*nbolo+ib]+=qri*dthetai[ib+k*nbolo]+qrq*dthetaq[ib+k*nbolo]+qru*dthetau[ib+k*nbolo];
      }
    }
     ...
     //}
      ...
      if (rank==0) {
        ...
        if (Param->FITTHETA&&itbogo>0) {
          ...
          for (i=0;i<nbolo;i++) 
            soff+=hit2[newnr[nbolo]+nmatdust+nmatco+npixbeam*nbolo]*
              d2[newnr[nbolo]+nmatdust+nmatco+npixbeam*nbolo+i];
          for (i=0;i<nbolo;i++) q2[newnr[nbolo]+nmatdust+nmatco+npixbeam*nbolo+i]=soff;
        }
      }
      for (k=0;k<nnbpix;k++) if (flgpix[k]>0) {
        ...
        if (ndata>0) {
          ...
          for (ib=0;ib<nbolo;ib++) {
            ...
            if (Param->FITTHETA&&itbogo>0) {
              vali+=dthetai[ib+k*nbolo]  *d2[newnr[nbolo]+nmatdust+nmatco+nfreefree+npixbeam*nbolo+ib];
              valq+=dthetaq[ib+k*nbolo]  *d2[newnr[nbolo]+nmatdust+nmatco+nfreefree+npixbeam*nbolo+ib];
              valu+=dthetau[ib+k*nbolo]  *d2[newnr[nbolo]+nmatdust+nmatco+nfreefree+npixbeam*nbolo+ib];
            } 
          }
          ...
          for (l1=0;l1<ndata;l1++) {
            ...
            if (flg_rg[htmp[l1].ib][ri1]!=0) {
              ...
              if (Param->FITTHETA&&itbogo>0) 
                val2+=d2[newnr[nbolo]+nmatdust+nmatco+npixbeam*nbolo+htmp[l1].ib]
                  *(-SI1*qmat[k]+CO1*umat[k]);
              ...
              ////////// THETA
              if (Param->FITTHETA&&itbogo>0) 
                q2[newnr[nbolo]+nmatdust+nmatco+nfreefree+npixbeam*nbolo+htmp[l1].ib]+=
                  ww*val2*(-SI1*qmat[k]+CO1*umat[k]);
          ...
          for (k=0;k<nnbpix;k++) if (flgpix[k]>0) {
            ...
            if (ndata>0) {
              ...
              for (ib=0;ib<nbolo;ib++) {
                ...
                if (Param->FITTHETA&&itbogo>0) {
                  vali+=dthetai[ib+k*nbolo]  *ix2[newnr[nbolo]+nmatdust+nmatco+npixbeam*nbolo+ib];
                  valq+=dthetaq[ib+k*nbolo]  *ix2[newnr[nbolo]+nmatdust+nmatco+npixbeam*nbolo+ib];
                  valu+=dthetau[ib+k*nbolo]  *ix2[newnr[nbolo]+nmatdust+nmatco+npixbeam*nbolo+ib];
                } 
              }
...
void minimize_gain_gi(double *ix2,double *gain,double *gaingi)
{
  ...
  int itermax = 500;
  int iter;
  double  delta_new, alpha, delta_old, beta;
  ...
  for (k=0;k<nnbpix;k++)  {
	...
	if (flg_rg[htmp[l1].ib][ri1]!=0) {
	  ...
	  if (Param->REMHDIP==0)  {
	    ...
	    dii[htmp[l1].gi+htmp[l1].ib*GAINSTEP ]+=htmp[l1].w*htmp[l1].freefree;
	    dqq[htmp[l1].gi+htmp[l1].ib*GAINSTEP ]+=htmp[l1].w*htmp[l1].freefree*CO1;
	    duu[htmp[l1].gi+htmp[l1].ib*GAINSTEP ]+=htmp[l1].w*htmp[l1].freefree*SI1;
	  }
	...
	if (flg_rg[htmp[l1].ib][ri1]!=0) {
	  ...
	  if (Param->REMHDIP==0)  {
	    ...
	    li=htmp[l1].w*htmp[l1].freefree;
	    lco=CO1*htmp[l1].w*htmp[l1].freefree;
	    lsi=SI1*htmp[l1].w*htmp[l1].freefree;
	  }
	...
	if (flg_rg[htmp[l1].ib][ri1]!=0) {
	  ...
	  if (Param->REMHDIP==0)  {
	    ...
	    cdip[htmp[l1].ib*GAINSTEP+htmp[l1].gi]+=htmp[l1].freefree;
	  }
  ...
  for (k=0;k<nnbpix;k++) if (flgpix[k]>0) {
	...
	if (flg_rg[htmp[l1].ib][ri1]!=0) {
	  ...
	  if (Param->REMHDIP==0)  {
	    ...
	    val2+=ix2[newnr[nbolo]+htmp[l1].ib*GAINSTEP+htmp[l1].gi]*htmp[l1].freefree;
	  }
	  ...
	  if (Param->REMHDIP==0)  {
	    ...
	    q2[newnr[nbolo]+htmp[l1].ib*GAINSTEP+htmp[l1].gi]+=ww*val2*htmp[l1].freefree;
	  }
  ...
  while ((iter < itermax)  && ((delta_new) > delta0*1E-24) && ((delta_new) > 1E-20)) //Param->XI2STOP))
	...
	if (ndata>0) {
	  ...
	  for (l1=0;l1<ndata;l1++) {
		...
		val2+=d2[newnr[nbolo]+htmp[l1].ib*GAINSTEP+htmp[l1].gi]*htmp[l1].freefree;
	      }
	      if (nmatco>0)   
  ...
  for (k=0;k<nnbpix;k++)  {
	...
	if (flg_rg[htmp[l1].ib][ri1]!=0) {
	  ...
	  if (Param->REMHDIP==0) {
	    ...
	    htmp[l1].lvi=htmp[l1].w*htmp[l1].freefree/II[k];
	  }
#endif
	}
      }
  ...
  while ((iter < itermax)  && ((delta_new) > delta0*1E-24) && ((delta_new) > 1E-20)) //Param->XI2STOP))
	...
	if (ndata>0) {
	  ...
	  for (ib=0;ib<nbolo;ib++) {
	    ...
	    ////////// THETA
	    if (Param->FITTHETA&&itbogo>0) 
	      q2[newnr[nbolo]+nmatdust+nmatco+nfreefree+npixbeam*nbolo+ib]+=qri*dthetai[ib+k*nbolo];
	  }
	}
      }
      if (rank==0) {
...
void minimize_adu(double *in_off,double *ix2,double *gain,double *gaingi)
{
}
...
void minimize_adu_nopol(double *in_off,double *ix2,double *gain,double *gaingi)
{
  ...
  for (k=0;k<nnbpix;k++)  {
	...
	if (flg_rg[htmp[l1].ib][ri1]!=0) {
	  ...
	  for (ir=0;ir<nbolo;ir++) {
	    ...
	    ///////////// THETA
	    if (Param->FITTHETA>0&&itbogo>0) {
	      b2[nmatdust+nmatco+npixbeam*nbolo+ir]+=ww*tmp*ctheta[ir];
	      hit2[nmatdust+nmatco+npixbeam*nbolo+ir]+=ww*ctheta[ir]*ctheta[ir];
	    }
	  }
	}
      }
  ...
  if (rank==0) {
    ...
    if (Param->FITTHETA&&itbogo>0) {
      soff=0;
      for (i=0;i<nbolo;i++) 
	soff+=hit2[nmatdust+nmatco+npixbeam*nbolo]*
	  ix2[nmatdust+nmatco+npixbeam*nbolo+i];
      for (i=0;i<nbolo;i++) q2[nmatdust+nmatco+npixbeam*nbolo+i]=soff;
    }
  }
  ...
  while ((iter < itermax)  && ((delta_new) > delta0*1E-24) && ((delta_new) > 1E-20)) //Param->XI2STOP))
	...
	if (Param->FITTHETA&&itbogo>0) {
	  ...
	  for (i=0;i<nbolo;i++) 
	    soff+=hit2[nmatdust+nmatco+npixbeam*nbolo]*
	      d2[nmatdust+nmatco+npixbeam*nbolo+i];
	  for (i=0;i<nbolo;i++) q2[nmatdust+nmatco+npixbeam*nbolo+i]=soff;
	}
      }
      for (k=0;k<nnbpix;k++) if (flgpix[k]>0) {
	...
	if (ndata>0) {
	  ...
	  for (ib=0;ib<nbolo;ib++) {
	    ...
	    if (Param->FITTHETA&&itbogo>0) {
	      vali+=dthetai[ib+k*nbolo]  *d2[nmatdust+nmatco+nfreefree+npixbeam*nbolo+ib];
	    } 
	  }
	  ...
	  for (ib=0;ib<nbolo;ib++) {
	    ...
	    ////////// THETA
	    if (Param->FITTHETA&&itbogo>0) 
	      q2[nmatdust+nmatco+nfreefree+npixbeam*nbolo+ib]+=qri*dthetai[ib+k*nbolo];
	  }
	}
      }
      if (rank==0) {
	...
	for (i=0; i < nmatres ; i++) ix2[i] += alpha * d2[i];
      ...
      gettimeofday(&tp2,NULL);
      if (rank==0&&iter%20==0) fprintf (stderr,"iter = %d - delta0 = %lg - delta_new = %lg %ld %3lfs\n", iter, delta0, delta_new,
      		    (long) testwrit,(double)(tp2.tv_sec-tp1.tv_sec)+(1E-6)*(tp2.tv_usec-tp1.tv_usec));
...
int PIOWriteMAP(const char *path,double *value_in_double,int beg,int end)
{
  ...
  if (rank==0) {
    ...
    int fp=open(path,O_WRONLY|O_CREAT,0664);
    err = write(fp,map,sizeof(float)*map_size);
    if (err == -1) {
      fprintf( stderr, "ERROR %d in sroll.c::PIOWriteMAP(filename=%s)\n", errno, path);
    } else if (err != sizeof(float)*map_size) {
      fprintf( stderr, "ERROR: only %ld bytes written out of %ld in sroll.c::PIOWriteMAP(filename=%s)\n", err, sizeof(float)*map_size, path);
    }
    close(fp);
    free(map);
  }
  ...
  MPI_Barrier(MPI_COMM_WORLD);
  sprintf(thepath,"%s_dir_%d/proc_%d",path,mpi_size,rank);
  int fp=open(thepath,O_WRONLY|O_CREAT,0664);
  err=write(fp,value,size);
  close(fp);
...
int PIOReadVECT(const char *path,void *value,int off,int size)
{
  int fp=open(path,O_RDONLY,0664);
  int err=pread(fp,value,size,off);
  close(fp);
  return(err);
}

int PIOReadVECTDOUBLE(const char *path,void *value,int off,int size)
{
  int fp=open(path,O_RDONLY,0664);
  int err=pread(fp,value,size*sizeof(double),off*sizeof(double));
  close(fp);
  return(err/sizeof(double));
}
...
int main(int argc,char *argv[])  {
  ...
  //fprintf(stderr,"* rank%d has detected mpi_size=%d\n", rank, mpi_size);

 
  /* read params */
  sroll_parContent par;
  par.flag_stim_paramfiles=0;
  par.REMHDIP=0;
  ...
  PIOLONG samples_per_rank = globalRangeRing / mpi_size;
  ...
  if (samples_per_rank == 0) {
    ...
    balancing_correction = globalRangeRing - (samples_per_rank * mpi_size);
  }

  if (rank==0) {
    fprintf(stderr,"[DBG] globalRangeRing      = "PIOLONG_FMT" \n", globalRangeRing);
    fprintf(stderr,"[DBG] samples_per_rank     = "PIOLONG_FMT" \n", samples_per_rank);
    fprintf(stderr,"[DBG] balancing_correction = "PIOLONG_FMT" \n", balancing_correction);
  }

  /* 2- Store result in structure */
  ...
  for (irank = 0; irank < mpi_size; irank++) {
    ...
    PIOLONG range = samples_per_rank + sup;
  ...
  if (rank==0) {
    int i;
    for (i = 0; i < mpi_size; i++) {
      fprintf(stderr,"  rank#%d begin:"PIOLONG_FMT" end:"PIOLONG_FMT"\n", i,
          globalRankInfo.BeginRing[i], globalRankInfo.EndRing[i]);
    }
  }
  ...
  PIOLONG ib;
  double *eta=(double *) malloc(sizeof(double)*nbolo);
  ...
  if (Param->flag_Theo_FREEFREE==_PAR_TRUE) {
    ...
    if (nsa<0) {
      fprintf(stderr, "Impossible to read %s %d\n",Param->Theo_FREEFREE,(int) nsa);
  ...
  for (ib=0;ib<nbolo;ib++) {
    ...
    if (Param->flag_stim_paramfiles == 1) {
      ...
      if (rank==0) {
        assert( stim_readParam( &stimPar.Param, Param->stim_paramfiles[ib]) == 0
                && "sroll.c: error while reading stim() parameter file");
        readBRIMO( stimPar.Param.brimo_filename, getBC( stimPar.Param.bolometer), &stimPar.brimo);
      }
      assert( MPI_Bcast( &stimPar, sizeof( stimPar), MPI_BYTE, 0, MPI_COMM_WORLD) == MPI_SUCCESS);
      MPI_Comm_size( MPI_COMM_WORLD, &stimPar.mpi_size);
      MPI_Comm_rank( MPI_COMM_WORLD, &stimPar.mpi_rank);
      sprintf( stimPar.msg_prefix, "rank %.02d/%.02d:", stimPar.mpi_rank, stimPar.mpi_size);
    }
    ...
    if (Param->TESTPOL==0) {
      ...
      close(fp);
      if (err<0) {
        fprintf(stderr,"Do not succeed to read %s %d %ld\n",Param->Signal_noPS[2*nbolo+ib],err,(long) nnoisesim);
        exit(-1);
      }
    ...
    badring = (PIOINT *) malloc(sizeof(PIOINT)*(globalRankInfo.EndRing[rank]-globalRankInfo.BeginRing[rank]+1));
    if (badring == NULL) {
    ...
    int tperr = noDMC_readObject_PIOINT(Param->Badring[ib],globalRankInfo.BeginRing[rank],globalRankInfo.EndRing[rank]-globalRankInfo.BeginRing[rank]+1,badring);
    if (tperr<0) {
      fprintf(stderr, "Impossible to read BAD RING %s %d\n",Param->Badring[ib],tperr);
      
      exit ( -1);
    }

    PIOFLOAT *stim_hpr = NULL;
    if (Param->flag_stim_paramfiles == 1) {
      // call stim() on full ring interval for bolometer <ib>
      int begin_ring = globalRankInfo.BeginRing[rank];
      int end_ring   = globalRankInfo.EndRing[rank];
      stimPar.Param.rings_per_read = end_ring - begin_ring + 1;
      stim_hpr = (PIOFLOAT *) malloc( sizeof( PIOFLOAT) * RINGSIZE * stimPar.Param.rings_per_read);
      assert( stim_hpr != NULL);
      printf( "sroll: rank %d/%d calling stim() on rings %d-%d (%e samples)\n",
              stimPar.mpi_rank, stimPar.mpi_size, begin_ring, end_ring, (float) (ENDRINGINDEX(end_ring) - BEGINRINGINDEX(begin_ring) + 1));
      stim( &stimPar, begin_ring, stimPar.Param.rings_per_read, stim_hpr);
    }
    ...
    for (rg=globalRankInfo.BeginRing[rank];rg<=globalRankInfo.EndRing[rank];rg+=step) {
      if (badring[rg-globalRankInfo.BeginRing[rank]]==0) {
        ...
        if (tperr<0) {
          fprintf(stderr, "Impossible to read %s HIT %d\n",Param->Hit_noPS[ib],tperr);
          exit ( -1);
        }
        ...
        if (tperr<0) {
          fprintf(stderr, "Impossible to read %s FSL %d\n",Param->fsl[ib],tperr);
          exit ( -1);
        }

        if (Param->flag_stim_paramfiles == 0) {
          y = (PIOFLOAT *) malloc(sizeof(PIOFLOAT)*RINGSIZE);
          assert( (y != NULL) && "Not enough memory to allocate signal HPR array");
          tperr = noDMC_readObject_PIOFLOAT(Param->Signal_noPS[ib],rg*RINGSIZE,RINGSIZE,y);
          if (tperr<0) {
            fprintf(stderr, "Impossible to read %s SIGNAL %d\n",Param->Signal_noPS[ib],tperr);
            exit ( -1);
          }
        } else {
          y = &stim_hpr[RINGSIZE * (rg - globalRankInfo.BeginRing[rank])];
        }
        ...
        if (tperr<0) {
          fprintf(stderr, "Impossible to read %s DIPORB %d\n",Param->DipOrb_noPS[ib],tperr);
        ...
        for (i=0;i<npixbeam;i++) {
          ...
          if (tperr<0) {
            fprintf(stderr, "Impossible to read %s THEO %d\n",Param->Theo_noPS[i*nbolo+ib],tperr);
        ...
        if (Param->flag_ADU==_PAR_TRUE) {
          ...
          if (tperr<0) {
            fprintf(stderr, "Impossible to read %s ADU %d\n",Param->ADU[ib],tperr);
        ...
        if (Param->TESTPOL==0) {
          ...
          for (i=0;i<ntoisample;i++) {
            if (posnoise[i]>=0&&posnoise[i]<RINGSIZE) {
              if (h[posnoise[i]]>0) {
                y[posnoise[i]]+=tpout[2*i]/h[posnoise[i]];
              }
            }
          }
          free(posnoise);
          ...
          for (i=0;i<RINGSIZE;i++) {
            ...
            y[i] /= Param->Calibration[nbolo+ib*4]+
...
#define COMPPHASE
#ifndef COMPPHASE
        tperr = PIOReadPOIObject((void **) &phase,Param->phase[ib],"PIODOUBLE",Command,NULL);
        if (tperr<0) {
          fprintf(stderr, "Impossible to read %s %d\n",Param->phase[ib],tperr);
	    ...
	    if (dustmapI!=NULL) {
	      if (dustmapI[ipix128]>-1E10) testmap=1;
            ...
            if (testmap) {
              ...
              tp_hpix->sig=(y[i]-Param->Monop[ib])/Param->Calibration[ib];
...
#if 1
	      if (dustmapI!=NULL&&Param->OUT_NOPOL[0]==1) {
		tp_hpix->dustmap=tp_hpix->co*comapI[ipix128]
		  +tp_hpix->si*dustmapI[ipix128];
		//tp_hpix->sig-=tp_hpix->co*comapI[ipix128]
		//+tp_hpix->si*dustmapI[ipix128];
		tp_hpix->comap=-tp_hpix->si*comapI[ipix128]
		  +tp_hpix->co*dustmapI[ipix128];
	      }
#endif
    ...
    for (js=0;js<mpi_size;js+=exch) {
      ...
      for (k=0;k<exch;k++) {
        if (rank%exch==k) {
          ...
          GetProcMem(&vmem,&phymem);
          if (rank==0) fprintf(stderr,"Rank: %ld[%d/%d] MEM %.1lf[%.1lf]MB line=%d %ld\n",
                  (long) rank, (int) (rk0+(k+exch/2)%(exch)),(int) exch,
                  (double) vmem/1024./1024.,
...
//                k+begpix[rank], II, IQ, IU, QQ, QU, UU);
 ...
 //========================================================================
  ...
  cdeg= (double *) malloc(sizeof(double)*nbolo*DEGREE);
  cpix= (double *) malloc(sizeof(double)*nbolo*npixbeam);
  ...
  long maxsizemat=newnr[nbolo]+nbolo*(GAINSTEPADU+npixbeam)+nmatco+nmatdust+nfreefree;
  if (GAINSTEP>GAINSTEPADU) maxsizemat=newnr[nbolo]+nbolo*(GAINSTEP+npixbeam)+nmatco+nmatdust+nfreefree;
  ...
  long nseed;
  for (nseed=0;nseed<Param->n_SEED;nseed++) {
    ...
    if (rank==0) fprintf(stderr,"SEED %ld\n",(long) Param->SEED[nseed]);
  ...
  if (rank==0) {
    ...
    for (i=0;i<nbolo;i++) {
      sprintf(saveg,"%s_OFF",Param->Out_Offset[i]);
      fprintf(stderr,"Read OFF  %lld\n",(long long) PIOReadVECTDOUBLE(saveg,tmpoff,globalBeginRing,globalRangeRing));
      for (j=newnr[i];j<newnr[i+1];j++) x3[j]=tmpoff[rgordinv[i][j-newnr[i]]];

      sprintf(saveg,"%s_GAIN",Param->Out_Offset[i]);
      fprintf(stderr,"Read GAIN  %lld\n",(long long) PIOReadVECTDOUBLE(saveg,tmpoff,globalBeginRing,globalRangeRing));
      for (j=0;j<globalRangeRing;j++) gain[invgi[j+globalBeginRing+i*32000]+GAINSTEP*i]=tmpoff[j];

#if 0
      sprintf(saveg,"%s_GADU",Param->Out_Offset[i]);
      fprintf(stderr,"Read GADU  %lld\n",(long long) PIOReadVECTDOUBLE(saveg,gainadu+i*GAINSTEPADU,0,GAINSTEPADU));
#endif

    }
    free(tmpoff);

    sprintf(saveg,"%s_X2",Param->Out_Offset[0]);
    //for (i=newnr[nbolo]+GAINSTEP*nbolo;i<nmatres;i++) fprintf(stderr,"X2 %d %lg\n",(int) i,x3[i]);
    fprintf(stderr,"Save X2 NO CUTRG\n");
    fprintf(stderr,"Read MAT  %lld\n",(long long) PIOReadVECTDOUBLE(saveg,x3+newnr[nbolo],0,nbolo*(GAINSTEP+npixbeam)+nmatdust+nmatco+nfreefree));
  }
  
  MPI_Bcast(x3,sizeof(double)*(newnr[nbolo]+nbolo*(GAINSTEP+npixbeam)+nmatdust+nmatco+nfreefree), MPI_BYTE, 0, MPI_COMM_WORLD);
  MPI_Bcast(gain,sizeof(double)*(nbolo*GAINSTEP), MPI_BYTE, 0, MPI_COMM_WORLD);
  ...
  if (Param->FITTHETA&&itbogo>0) {
    for (i=0;i<nbolo;i++) 
      dpsisi[i]+=x3[newnr[nbolo]+nbolo*(DEGREE+GAINSTEP)+nmatdust+nmatco+nfreefree+npixbeam*nbolo+i];
  }
  ...
  for (i=0;i<6;i++) {
    NMAPS[i] = (double *) malloc(sizeof(double)*nnbpix);
    memset(NMAPS[i],0,sizeof(double)*nnbpix);
  }
  for (i=0;i<6;i++) {
    YNMAPS[i] = (double *) malloc(2*sizeof(double)*nnbpix);
    memset(YNMAPS[i],0,2*sizeof(double)*nnbpix);
  }

  for (i=0;i<3;i++) {
    MAPS_CORR[i] = (double *) malloc(sizeof(double)*nnbpix);
    YMAPS_CORR[i] = (double *) malloc(2*sizeof(double)*nnbpix);
  }
  GetProcMem(&vmem,&phymem);
  MPI_Barrier(MPI_COMM_WORLD);
  ...
  for (isurv=0;isurv<5;isurv++) {
	...
	for (i=0;i<ndata;i++) {
	  ...
	  ib=htmp[i].ib;
	  if (flg_rg[ib][ri]!=0&&Param->bolomask[maptype*nbolo+htmp[i].ib]==1&&(htmp[i].surv=isurv||isurv==0)) {
	    ri=rgord[ib][ri]+newnr[htmp[i].ib];
	    if (CUTRG>1) ri2=rgord2[ib][ri2]+newnr2[htmp[i].ib];
	    
	    double gg2=gain[htmp[i].gi+htmp[i].ib*GAINSTEP];
	    
	    double sig_corr = htmp[i].sig*gg2-htmp[i].fsl-htmp[i].dip-gainadu[htmp[i].adu+htmp[i].ib*GAINSTEPADU];
	    
	    if (CUTRG>1) sig_corr-=xoff[ri2];
	    else sig_corr-=x3[ri];
	    ...
	    if (htmp[i].surv<10) { // HM1
	      
	      YMAPS_CORR[0][k] +=htmp[i].w*sig_corr;
	      YMAPS_CORR[1][k] +=htmp[i].w*sig_corr*CO1;
	      YMAPS_CORR[2][k] +=htmp[i].w*sig_corr*SI1;
	      
	      YNMAPS[0][k  ] +=htmp[i].w;
	      YNMAPS[1][k  ] +=htmp[i].w*CO1;
	      YNMAPS[2][k  ] +=htmp[i].w*SI1;
	      YNMAPS[3][k  ] +=htmp[i].w*CO1*CO1;
	      YNMAPS[4][k  ] +=htmp[i].w*CO1*SI1;
	      YNMAPS[5][k  ] +=htmp[i].w*SI1*SI1;
	    }
	    else {// HM2
	      
	      YMAPS_CORR[0][k+nnbpix] +=htmp[i].w*sig_corr;
	      YMAPS_CORR[1][k+nnbpix] +=htmp[i].w*sig_corr*CO1;
	      YMAPS_CORR[2][k+nnbpix] +=htmp[i].w*sig_corr*SI1;
	      
	      YNMAPS[0][k  +nnbpix] +=htmp[i].w;
	      YNMAPS[1][k  +nnbpix] +=htmp[i].w*CO1;
	      YNMAPS[2][k  +nnbpix] +=htmp[i].w*SI1;
	      YNMAPS[3][k  +nnbpix] +=htmp[i].w*CO1*CO1;
	      YNMAPS[4][k  +nnbpix] +=htmp[i].w*CO1*SI1;
	      YNMAPS[5][k  +nnbpix] +=htmp[i].w*SI1*SI1;
	    }
	    ...
	    NMAPS[0][k  ] +=htmp[i].w;
	    NMAPS[1][k  ] +=htmp[i].w*CO1;
	    NMAPS[2][k  ] +=htmp[i].w*SI1;
	    NMAPS[3][k  ] +=htmp[i].w*CO1*CO1;
	    NMAPS[4][k  ] +=htmp[i].w*CO1*SI1;
	    NMAPS[5][k  ] +=htmp[i].w*SI1*SI1;
	  }
	}  
      }
	...
	if (NMAPS[0][k]>0) {
	  if (Param->OUT_NOPOL[maptype]==1) {
	    MAPS_CORR[0][k]/=NMAPS[0][k];
	    MAPS_CORR[1][k]=UNSEENPIX;
	    MAPS_CORR[2][k]=UNSEENPIX;
	  }
	  else {
	    solvemap(MAPS_CORR[0]+k,MAPS_CORR[1]+k,MAPS_CORR[2]+k,
		     NMAPS[0][k],NMAPS[1][k],NMAPS[2][k],
		     NMAPS[3][k],NMAPS[4][k],NMAPS[5][k]);
	    double cond=cond_3_3_thres(NMAPS[0][k],NMAPS[1][k],NMAPS[2][k],
				       NMAPS[1][k],NMAPS[3][k],NMAPS[4][k],
				       NMAPS[2][k],NMAPS[4][k],NMAPS[5][k]);
	    if (cond >= Param->seuilcond) {
	      for (p=0;p<6;p++) NMAPS[p][k]=0;
	      for (p=0;p<3;p++) {
	...
	for (ll=0;ll<2;ll++) {
	  ...
	  if (YNMAPS[0][kk]>0) {
	    if (Param->OUT_NOPOL[maptype]==1) {
	      YMAPS_CORR[0][kk]/=YNMAPS[0][kk];
	      YMAPS_CORR[1][kk]=UNSEENPIX;
	      YMAPS_CORR[2][kk]=UNSEENPIX;
	    }
	    else {
	      solvemap(YMAPS_CORR[0]+kk,YMAPS_CORR[1]+kk,YMAPS_CORR[2]+kk,
		       YNMAPS[0][kk],YNMAPS[1][kk],YNMAPS[2][kk],
		       YNMAPS[3][kk],YNMAPS[4][kk],YNMAPS[5][kk]);
	      double cond=cond_3_3_thres(YNMAPS[0][kk],YNMAPS[1][kk],YNMAPS[2][kk],
					 YNMAPS[1][kk],YNMAPS[3][kk],YNMAPS[4][kk],
					 YNMAPS[2][kk],YNMAPS[4][kk],YNMAPS[5][kk]);
	      if (cond >= Param->seuilcond) {
		for (p=0;p<6;p++) YNMAPS[p][kk]=0;
		for (p=0;p<3;p++) {
		  YMAPS_CORR[p][kk]=UNSEENPIX;
		}
	      }
	    }
	  }
	...
	if (Param->OUT_NOPOL[maptype]!=1||p==0) {
	  ...
	  sprintf(comm,"begin=%lld;end=%lld",(long long) begpix[rank],(long long) begpix[rank]+nnbpix-1);
	  
	  PIOSTRING OUTMAP;
	  sprintf(OUTMAP,"%s_survey_%ld_%s",mapout[maptype],isurv,suffix);
	  PIOWriteMAP(OUTMAP,NMAPS[p],begpix[rank],begpix[rank]+nnbpix-1);
	  //if (rank==0) PIOMergeMAP(OUTMAP);
	  MPI_Barrier(MPI_COMM_WORLD);
	  if (isurv==0) {
	    for (i=0;i<2;i++) {
	      sprintf(OUTMAP,"%s_year_%d_%s",mapout[maptype],(int) i,suffix);
	      PIOWriteMAP(OUTMAP,YNMAPS[p]+i*nnbpix,begpix[rank],begpix[rank]+nnbpix-1);
	      //if (rank==0) PIOMergeMAP(OUTMAP);
	      MPI_Barrier(MPI_COMM_WORLD);
	    }
	  }
	...
	if (Param->OUT_NOPOL[maptype]!=1||p==0) {
	  ...
	  PIOSTRING OUTMAP;
	  sprintf(OUTMAP,"%s_corr_survey_%ld_%s",mapout[maptype],isurv,suffix);
	  ...
	  if (isurv==0) {
	    for (i=0;i<2;i++) {
	      ...
	      PIOWriteMAP(OUTMAP,YMAPS_CORR[p]+i*nnbpix,begpix[rank],begpix[rank]+nnbpix-1);
	      //if (rank==0) PIOMergeMAP(OUTMAP);
	      MPI_Barrier(MPI_COMM_WORLD);
	    }
	  }
      ...
      if (rank==0) fprintf(stderr,"%d %d %s\n",__LINE__,rank,__FILE__);
      
      MPI_Barrier(MPI_COMM_WORLD);
    }
  }

  if (rank%64==0) fprintf(stderr,"Finalize %d \n",rank);
