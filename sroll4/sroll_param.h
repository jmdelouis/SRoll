...
typedef struct {
  ...
  /* ---- Optional Parameter AVGCO143 ----------------------------------
   * when using bolometers from different frequencies,
   * if AVGCO143 is given, CO contribution will be forced to AVGCO143 value
   * for 143GHz bolometer and free for the others
   * eg: AVGCO143 = 0.0
   */
  PIODOUBLE AVGCO143;
  PIOBYTE flag_AVGCO143;  /* = 1 if AVGCO143 is present */
   ...
   * if set to 0, covariance matrices (II, IQ, IU, QQ, QU, UU) won't be saved
   * if omitted (default), cov matrices will be saved
   */
  PIOINT saveCOV;
