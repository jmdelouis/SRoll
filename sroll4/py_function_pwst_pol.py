...
def model(parameter,netinfo,rank):
  ...
  return modpar
# ----------------------------------------------------------------------------------------
def hpwst(image1,image2,mask,widx,widx2,first=True,doheavy=False):
    ...
    for iscale in range(nstep):
        ...
        if first==True and iscale<nstep-1:
            val2=(hpwst(tf.nn.relu(tmp),tf.nn.relu(tmp),vmask,widx,widx2,first=False,doheavy=doheavy)-hpwst(tf.nn.relu(-tmp),tf.nn.relu(-tmp),vmask,widx,widx2,first=False,doheavy=doheavy))
            if iscale==0:
...
def init_shape(nbolo,xsize,ysize,rank,
  ...
  return(mynetwork)
# ----------------------------------------------------------------------------------------
def init_shape_from_files(TFLEARN,bolotab,rank,CNN_LEARN_MODEL,tmpname,MAP_CNN_path,FSL_CNN_path):
    """ 
    ...
    for elt in CNN_names:
      #load netcdf
      if(elt == 'MAP'):
        netcdf = Dataset(MAP_CNN_path+'/MAP_CNN_netcdf.nc','r', format='NETCDF4')
      else:
        netcdf = Dataset(FSL_CNN_path+'/FSL_CNN_netcdf.nc','r', format='NETCDF4')
      ...
      mynetwork[elt]['PAR'] = netcdf[elt].variables['mappar'][:]
      ...
      if elt=='MAP':
        mynetwork[elt]['crossl']=(CNN_LEARN_MODEL//100)%16
      
      if rank==0:
        sys.stderr.write('NPAR PER IMAGE %d %d %d\n'%(mynetwork[elt]['XSCALE'],mynetwork[elt]['YSCALE'],NDCONV))
      ...
      DEEPNESS = netcdf[elt].dimensions['deepness'].size
...
def get_flb(myrun):
  ...
  return(flb)
# ---------------------------------------------------------------------------------------- 
def init_network(mynetwork,signal,weights,TCO1,TSI1,hidx,idx,realpix,coef,smap): #,in_param,in_convw,in_convb,in_flw,in_flb):
    nonet = False
    netinfo={}
    nmem=0.0
    ...
    for i in range(nstep):
        lout=nout//(2**i)
        #path en dur for debug
        
        #tmp=((np.load('/export/home/jmdeloui/heal_cnn/WIDXR_%d.npy'%(lout)).flatten()).astype('int32')).reshape(12*lout**2,8)
        tmp=((np.load(mynetwork['MAP']['TFLEARN']+'/WIDXR_%d.npy'%(lout)).flatten()).astype('int32')).reshape(12*lout**2,8)
        #print(tmp.shape)
      ...
      if mynetwork[elt]['NFC']==True:
        ...
        if mynetwork[elt]['LEARNPARAM']==True:
          if elt=='MAP':
            ...
            params[elt] = tf.Variable(mynetwork[elt]['PAR'].reshape(mynetwork[elt]['BATCHSZ'],mynetwork[elt]['NTOTHIDDEN']),name='PARAM_%s'%(elt))
          else:
          ...
          if elt=='MAP':
            ...
            params[elt] = tf.constant(mynetwork[elt]['PAR'].reshape(mynetwork[elt]['BATCHSZ'],mynetwork[elt]['NTOTHIDDEN']))
          else:
    ...
    map1=smap[0:12*32*32*2].reshape(1,12*32*32,1,2)
    map2=smap[12*32*32*2:].reshape(1,12*32*32,1,2)
    mask=((map1>-1E20)*(map2>-1E20)).astype('float32')
    map1[mask==0]=0
    map2[mask==0]=0
    imap1=tf.Variable(map1.astype('float32'))
    imap2=tf.Variable(map2.astype('float32'))
   ...
   #   import healpy as hp
   #   plt.figure()
   #   hp.mollview(map1[0,:,0,0],cmap='jet',norm='hist',hold=False,sub=(2,2,1),nest=True)
   #   hp.mollview(map1[0,:,0,1],cmap='jet',norm='hist',hold=False,sub=(2,2,2),nest=True)
   #   hp.mollview(map2[0,:,0,0],cmap='jet',norm='hist',hold=False,sub=(2,2,3),nest=True)
   #   hp.mollview(map2[0,:,0,1],cmap='jet',norm='hist',hold=False,sub=(2,2,4),nest=True)
   #   plt.figure()
   #   hp.mollview(mask[0,:,0,0],cmap='jet',norm='hist',hold=False,sub=(2,1,1),nest=True)
   #   hp.mollview(mask[0,:,0,1],cmap='jet',norm='hist',hold=False,sub=(2,1,2),nest=True)
   #   plt.show()
      
    mask=tf.constant(mask)
    refcross1,refcross2,j1,j2=hpwst(imap1,imap2,mask,widx,widx2)
    ...
    try:
      ...
      print("DEBUG",coef,netinfo["FSL"]['BATCHSZ'])
    ...
    incross1,incross2,j1,j2=hpwst(tf.reshape(outmap,[1,netinfo["MAP"]['XIMAGE_SIZE'],1,2]),tf.reshape(outmap,[1,netinfo["MAP"]['XIMAGE_SIZE'],1,2]),mask,widx,widx2)
  
    losscross=tf.reduce_sum(tf.square(refcross1-incross1))+tf.reduce_sum(tf.square(refcross2-incross2))
    ...
    if mynetwork['MAP']['crossl']==0:
      if mynetwork['MAP']['rank'] == 0:
        print('cross WST loss weight ',0 )
    else:
      if mynetwork['MAP']['rank'] == 0:
        print('cross WST loss weight ',10**(mynetwork['MAP']['crossl']-6) )
      loss=loss+losscross*(10**(mynetwork['MAP']['crossl']-6)) 
    ...
    myrun['refcross1']=refcross1
    myrun['refcross2']=refcross2
    myrun['assign']=assign
...
# ---------------------------------------------------------------------------------------- 
def init_network_data(myrun,signal,weights,hidx,realpix,smap): 
    ...
    map1=smap[0:12*32*32*2].reshape(1,12*32*32,1,2)
    map2=smap[12*32*32*2:].reshape(1,12*32*32,1,2)
    myrun["imap1"].assign(map1.astype('float32'))
    myrun["imap2"].assign(map2.astype('float32'))
...
def get_loss(myrun):
    sess=myrun['sess']
    #sess=tf.Session()
    l,lc,lr=sess.run([myrun['loss'],myrun['losscross'],myrun['learning_rate']])
    res=np.zeros([4],dtype='float32')
    res[0]=l
    res[1]=myrun['regul']
    res[2]=lr
    res[3]=lc #should the amplitude of the map
    return(res)
# ---------------------------------------------------------------------------------------- 
