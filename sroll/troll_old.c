...
// ##############################################################################################################################################################################################
/*
void Proj(hpix *htmp,float * x,int ndata,float SI,double nmatres ){
   
  double wp  = 0.0;
  for(int l1=0;l1<ndata;l1++){
      //calcul weights  
      for(int i=0;i<nmatres;i++){
        wp += x[i]+x[(int)nmatres]*htmp[l1].t;
      }
      //calcul R_ij
      htmp[l1].model_proj = wp - SI; 
  }
}*/

// -------------------------------------------------------------------------------------------------------------
void Proj_t(double SI,int nnbpix){
  ...
  for(int pix =0;pix<nnbpix;pix++){
    ...
    for(int l1=0;l1<ndata;l1++) {
      htmp[l1].t_ij = htmp[l1].t - SI;
    }
  }
}
// -------------------------------------------------------------------------------------------------------------
void proj_data(double *b2,int nnbpix,int rank,double nmatres){
  ...
  for(int pix =0;pix<nnbpix;pix++){
    ...
    double *tab_ti = (double *) malloc(sizeof(double)*(nbolo*npixbeam));
    memset(tab_ti,0,sizeof(double)*(nbolo*npixbeam));
    ...
    if (ndata>1&&flgpix[pix]>0) {
     //calcul tab_ti
      for(int l1=0;l1<ndata;l1++){
       long ri1=htmp[l1].rg-globalBeginRing;
        if (flg_rg[htmp[l1].ib][ri1]!=0) {
          for(int m =0;m<npixbeam;m++){
            tab_ti[htmp[l1].ib+nbolo*m]+= htmp[l1].w*htmp[l1].listofpix[m];
          }
        }
      }  
      ...
      for(int l1=0;l1<ndata;l1++){
        ...
        if (flg_rg[htmp[l1].ib][ri1]!=0) {
          ...
          b2[ir]+= htmp[l1].R_ij - htmp[l1].alpha*sum_Rij;   


          for(int m=0;m<npixbeam;m++){
            b2[newnr[nbolo]+htmp[l1].ib+m*nbolo]+= htmp[l1].listofpix[m]*(htmp[l1].R_ij - htmp[l1].alpha*sum_Rij);
          }     
        }
      }
    ...
    /*
      ...
      */  
      ...
      for(int m = 0;m<npixbeam;m++){            
        for(int b =0;b <nbolo;b++){
          b2[newnr[nbolo]+b+nbolo*m] -= tab_ti[b+m*nbolo] * sum_Rij;           
        } 
      }
      */
      ...
      for(int l1=0;l1<ndata;l1++){
        ...
        if (flg_rg[htmp[l1].ib][ri1]!=0) {
          ...
          double wp = htmp[l1].R_ij - htmp[l1].alpha*sum_Rij;         
          b2[ir]+= wp;          
      ...
      for(int l1=0;l1<ndata;l1++){
        ...
        if (flg_rg[htmp[l1].ib][ri1]!=0) {
            ...
            for(int m = 0;m<npixbeam;m++){
            for(int b =0;b <nbolo;b++){
              b2[newnr[nbolo]+b+nbolo*m] += - htmp[l1].R_ij* tab_ti[b+nbolo*m];
            }
            b2[newnr[nbolo]+htmp[l1].ib+nbolo*m] += htmp[l1].R_ij * htmp[l1].listofpix[m];
          }  
          //

         for(int m = 0;m<npixbeam;m++){
            //b2[newnr[nbolo]+htmp[l1].ib+nbolo*m] += htmp[l1].R_ij * (htmp[l1].listofpix[m] - tab_ti[htmp[l1].ib+nbolo*m])* sum_Rij;
            b2[newnr[nbolo]+htmp[l1].ib+nbolo*m] += htmp[l1].R_ij *htmp[l1].listofpix[m] - tab_ti[htmp[l1].ib+nbolo*m]* sum_Rij;
         }
        }
      }
...
// -------------------------------------------------------------------------------------------------------------
void proj_grad(double * x,double nmatres,double * q2,int nnbpix,int rank){
  ...
  for(int pix =0;pix<nnbpix;pix++){
     ...
     //calcul tab_ti
      ...
      for(int l1=0;l1<ndata;l1++){  
        ...
        if (flg_rg[htmp[l1].ib][ri1]!=0) {
          ...
          val = x[ir];
          for(int m = 0;m<npixbeam;m++){
            irt = newnr[nbolo]+htmp[l1].ib+nbolo*m;
            val+= x[irt]*htmp[l1].listofpix[m];
          }
          s_X+= val*htmp[l1].w;
        }
      }
      ...
      for(int l1=0;l1<ndata;l1++){
        ...
        if (flg_rg[htmp[l1].ib][ri1]!=0) {
          ...
          wp = (val-s_X)-htmp[l1].alpha*R_ij_bis;
          q2[ir]+= wp;
          for(int m=0;m<npixbeam;m++){
            q2[newnr[nbolo]+htmp[l1].ib+m*nbolo]+= htmp[l1].listofpix[m]*((val-s_X) -htmp[l1].alpha*R_ij_bis);
          }
        }
      }
    ...
    /*
      ...
      for(int m = 0;m<npixbeam;m++){            
        for(int b =0;b <nbolo;b++){
          q2[newnr[nbolo]+b+nbolo*m] -= tab_ti[b+m*nbolo] * R_ij_bis;           
        } 
      }
  */
      ...
      for(int l1=0;l1<ndata;l1++){
        ...
        if (flg_rg[htmp[l1].ib][ri1]!=0) {
          ...
          wp = (val-s_X)-htmp[l1].alpha*R_ij_bis;//htmp[l1].R_ij-htmp[l1].alpha*R_ij_bis;
          q2[ir]+= wp;
          ...
          for(int m = 0;m<npixbeam;m++){
            for(int b =0;b <nbolo;b++){
              q2[newnr[nbolo]+b+nbolo*m] += - (val-s_X)* tab_ti[b+nbolo*m];
            }
            q2[newnr[nbolo]+htmp[l1].ib+nbolo*m] += (val-s_X) * htmp[l1].listofpix[m];
        ...
        for(int m = 0;m<npixbeam;m++){
            //q2[newnr[nbolo]+htmp[l1].ib+nbolo*m] += (val-s_X)* (htmp[l1].listofpix[m] - tab_ti[htmp[l1].ib+nbolo*m] )* R_ij_bis);
            q2[newnr[nbolo]+htmp[l1].ib+nbolo*m] += (val-s_X)*(htmp[l1].listofpix[m] - tab_ti[htmp[l1].ib+nbolo*m]* R_ij_bis);
         } 
  ...
  if(rank == 0){
    ...
    for(int m =0;m<npixbeam;m++){
      double sum2 = 0.0;
      for(int b = 0;b<nbolo;b++){
        sum2+=x[newnr[nbolo]+b+m*nbolo]*1E5;
        fprintf(stderr,"sum2 =%lg \t x[%d] = %lg\n",sum2,newnr[nbolo]+b+m*nbolo,x[newnr[nbolo]+b+m*nbolo]);
      }
      
      for(int b = 0;b<nbolo;b++){
        q2[newnr[nbolo]+b+m*nbolo]+= sum2;
      }
...
void minimize_gain_nopol_tf(double *ix2,double *gaingi){
  //plip
  ...
  double *vec = (double*) malloc(sizeof(double)*(nmatres));
  
  double *matrice =(double *) malloc(sizeof(double)*(nmatres)*(nmatres));
  
   

  memset(matrice,0,sizeof(double)*nmatres*nmatres);
  memset(vec,0,sizeof(double)*nmatres);
  ...
  //calcul moyenne nulle 
  double v = 0.0;
  for(int i=0;i<newnr[nbolo];i++){
    v+=cos(i/1000.0);
  }
  v/=newnr[nbolo];
  ...
  for(int i=0;i<nmatres;i++) {
    //x_tab[i] = cos(i/1000.0)-v;
    x_tab[i] = 0.0;
  }

  
  x_tab[newnr[nbolo]] = -0.15;
  x_tab[newnr[nbolo]+1] = 0.15;
  x_tab[newnr[nbolo]+2] = -0.25;
  x_tab[newnr[nbolo]+3] = 0.25;
    ...
    if (ndata>0&&flgpix[k]>0) {
      ...
      for (l1=0;l1<ndata;l1++) {  // parcours des donnees du pixel
        ...
        if (flg_rg[htmp[l1].ib][ri1]!=0) {
          ...
          double val_tmp = cos(ir/1000.0)-v;
          for(int m = 0;m<npixbeam;m++){
            htmp[l1].listofpix[m] = rand();
            val_tmp += x_tab[newnr[nbolo]+htmp[l1].ib+m*nbolo] * htmp[l1].listofpix[m];
          }

          /*
          if (REMHDIP==0) val_tmp=(htmp[l1].sig*g1-htmp[l1].fsl-htmp[l1].dip-htmp[l1].corr_nl-htmp[l1].corr_cnn); // Calcul du signal 
          else val_tmp=(htmp[l1].sig*g1-htmp[l1].fsl-htmp[l1].freefree-htmp[l1].corr_nl-htmp[l1].corr_cnn);
          */       
      
          
          SI+=htmp[l1].w*val_tmp; // Calcul du signal 
      ...
      for (l1=0;l1<ndata;l1++) { // parcours des donnees du pixel
        ...
        if (flg_rg[htmp[l1].ib][ri1]!=0) {                  //if pixel ok
	        htmp[l1].R_ij = htmp[l1].m - SI;                 //Calcul R_ij   m - im/imh
	        htmp[l1].alpha = htmp[l1].w/II[k];              //calcul alpha        
        }
      }
      ...
      for (l1=0;l1<ndata;l1++) { // parcours des donnees du pixel
        ...
        long ir1 = rgord[htmp[l1].ib][ri1]+newnr[htmp[l1].ib];
        //calcul matrice
        matrice[ir1+ir1*nmatres] +=htmp[l1].alpha*htmp[l1].alpha;

        //DEBUG calcul matrice and vec  -----------------------------------
        for(int l2 = 0;l2<ndata;l2++){ 
          long ri2=htmp[l2].rg-globalBeginRing; 
          long ir = rgord[htmp[l2].ib][ri2]+newnr[htmp[l2].ib];
          if(ir!=ir1) matrice[ir + ir1 *nmatres] +=-htmp[l2].alpha*(htmp[l2].w/II[k]);
        }
        vec[ir1] += htmp[l1].alpha*htmp[l1].R_ij;
        // END debug -------------------------------------------------------
      }
      */
  ...
  {
    double *lb = (double *) malloc(sizeof(double)*(nmatres)*nmatres);
    MPI_Reduce(matrice,lb,nmatres*nmatres,MPI_DOUBLE,MPI_SUM,0,MPI_COMM_WORLD); 
    memcpy( matrice,lb,sizeof(double)*(nmatres)*nmatres); 
    free(lb);
  }
  ...
  {
    double *lb = (double *) malloc(sizeof(double)*(nmatres));
    MPI_Reduce(vec,lb,nmatres,MPI_DOUBLE,MPI_SUM,0,MPI_COMM_WORLD); 
    memcpy(vec,lb,sizeof(double)*(nmatres)); 
    free(lb);
  }
  ...
  Proj_t(SI,nnbpix);    //calcul t_ij          
  proj_data(b2,nnbpix,rank,nmatres); //calcul b2
  proj_grad(x_tab,nmatres,q2,nnbpix,rank); // Calcul q2 
  ...
  if (rank==0) {
    ...
    for (rrk=1;rrk<mpi_size;rrk++) {
      MPI_Recv(lb,sizeof(double)*(nmatres), MPI_BYTE, rrk,1031, MPI_COMM_WORLD,&statu);
      for (l=0;l<nmatres;l++) b2[l]+=lb[l];
      //fprintf(stderr,"PASS recv b2 rank = %d\n",rank);
    }
    free(lb);
  }
  else MPI_Send(b2, sizeof(double)*(nmatres), MPI_BYTE, 0, 1031, MPI_COMM_WORLD);
  ...
  if(rank == 0){
    for(int i =0;i<nmatres;i++) {
      fprintf(stderr,"q2[%d] =%lg \t b2[%d] = %lg \t hit2[%d] = %lg r2[%d] = %lg nmatres = %d rank = %d \n",i,q2[i],i,b2[i],i,hit2[i],i,r2[i],nmatres,rank);
    }
  }
  ...
  */
  ...
  //init
  double sum = 0.0 ,tmp = 0.0,alpha_tmp = 0.0 ,tmp_delta=0.0,beta= 0.0,delta = 0.0,alpha=0.0;
  ...
  //for(int n=0;n<2;n++){
      ...
      proj_grad(d2,nmatres,projX,nnbpix,rank);
      ...
      for(int k = 0;k < nmatres;k++){
        tmp_delta += r2[k]*r2[k];
        //tmp_delta +=tmp;
        //fprintf(stderr,"r2[%d] =%lg tmp_delta = %lg\n",k,r2[k],tmp_delta);
      }
      delta = tmp_delta;
      if(rank == 0) fprintf(stderr,"delta = %lg\n",delta);
      ...
      if(rank == 0) fprintf(stderr,"alpha = %lg \t alpha_tmp = %lg \n",alpha,alpha_tmp);
      ...
      for(int k = 0;k <nmatres;k++){
        new_r[k] = r2[k]-alpha*projX[k];
	      //if(rank == 0) fprintf(stderr," new_r[%d] = %lg \t r2[%d] = %lg \t projX[%d] = %lg  alpha*projX = %lg\n",k,new_r[k],k,r2[k],k,projX[k],alpha*projX[k]);
      }


      //test delta
      if(delta<tol){
        if(rank ==0) fprintf(stderr,"End delta = %lg\n",delta);
      ...
      if(rank == 0) fprintf(stderr,"beta = %lg sum = %lg \n",beta,sum,delta);      
      //if(rank == 0) for(int i= 0;i<10;i++) fprintf(stderr,"new_r[i] = %lg\n",i,new_r[i]);
      
      //calcul new_p
      for(int k =0;k<nmatres;k++){
        new_p[k] = new_r[k]+beta*d2[k];
      }
  ...
  if(rank == 0){
    ...
    fp=fopen("matrix","w+");
    for(int i = 0;i<nmatres;i++){      
      for(int j =0;j<nmatres;j++){
        //fprintf(fp,"m[%d,%d] = %lg \t",i,j,matrice[i+j*nmatres]);
        fprintf(fp,"%lg\t",matrice[i+j*nmatres]);
      }
      fprintf(fp,"\n");
    }
  ...
  */
  ...
  if(rank == 0){
    for(int i =0;i<nmatres;i++){
      double tmp = 0.0;
      for(int j =0;j<nmatres;j++){
        tmp += matrice[i+j*nmatres]*x_tab[j];
        fprintf(stderr,"matrice[%d,%d] = %lg \n",i,j,matrice[i+j*nmatres]);  
      }
      //fprintf(stderr,"tmp = %lg --- vec[%d] =%lg\n",tmp,i,vec[i]);   
    }
  }
  ------------------------------------------------------------------------------------------------- */
...
{ 
  ...
  int cnn_down = (2048/CNN_NSIDE)*(2048/CNN_NSIDE);
  for (i=0;i<nnbpix;i++){
    long ipix;
    ring2nest(2048,realpix[i],&ipix);
   ...
   // if (flgpix[k]>0) {
	...
	for (l1=0;l1<ndata;l1++) {
	  ...
	  if (flg_rg[htmp[l1].ib][ri1]!=0) {
	    ...
	    temp=0;
	    for (m=0;m<npixbeam;m++)  {
	      if (DOCNN[m]==0) { //||(DOCNN[m]!=1&&NORMFITPOL==0)) {
		sig_corr -= htmp[l1].listofpix[m]*x3[newnr[nbolo]+nbolo*(GAINSTEP)+m*nbolo+htmp[l1].ib];
	      }
	      else {
		if (DOCNN[m]!=2) {
  ...
  for (k=0;k<nnbpix;k++) {
	...
	for (m=0;m<npixbeam;m++)  {
	    if (DOCNN[m]==1) {
	      htmp[l1].listofpix[m]=sqrt(-2*log( drand48()))*cos(2*M_PI*drand48()); // should be set to something otherwise it generates Nan during the minimization
	    }
	    if (DOCNN[m]==2) {
	      htmp[l1].listofpix[m]=prediction[0][iii]; // should be set to something otherwise it generates Nan during the minimization
	    }
	    if (DOCNN[m]==3) {
	      htmp[l1].listofpix[m]=prediction[1][iii]; // should be set to something otherwise it generates Nan during the minimization
	    }
	}
...
//  Param->REMDIP = Param->REMDIP; // ??? commented by SM Nov2018
  ...
  if (GAINSTEPADU!=0) {
	...
	fprintf(stderr,"debug print => nbolo = %d mpi_size = %d \n",nbolo,mpi_size);
...
// npixbeam+5 SYNCROTRON
		...
		for (int iter = 0; iter < number_of_iterations; iter++) {
	      ...
	      if (addpol!=NULL) {
		for (int iter = 0; iter < number_of_iterations; iter++) {
		  tp_hpix->listp[iter]+=eta[ib]*(tp_hpix->co*addpol[2*l_ipix128]+tp_hpix->si*addpol[2*l_ipix128+1]);
		}
	      }
...
#define STEPRG_HISTOADU (128)
  ...
  for (int iter = 0; iter < number_of_iterations; iter++) {
    ...
    while (itt<Param->NITT) {
      ...
      if (Param->OUT_NOPOL[0]%2==1) minimize_gain_nopol_tf(x3,gain);
...
// nltemp could be removed
  ...
  if (Param->flag_DOCNN==_PAR_TRUE) {
    if (python_rank==0) MPI_Comm_free(&tensorflow_comm);
    MPI_Comm_free(&python_comm);
  }
