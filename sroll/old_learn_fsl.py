...
#Init 
parameter_FSL = np.load("/export/home/jmdeloui/heal_cnn/FPAR.npy")

#Nb nodes 

#fsl_N_in = 1
#fsl_N_out = 5
print("dconv weight shape ",dconv_weights[i].get_shape())
fsl_N_in = dconv_weights[i].get_shape()[2]
#fsl_N_out = dconv_weights[i].get_shape()[0]
fsl_N_out = dconv_weights[i].get_shape()[3]


fsl_node=(XIMAGE_SIZE*YIMAGE_SIZE)/NDCONV


fsl_grp = f.createGroup('FSL')

#Create dimensions FSL
fsl_grp.createDimension("f_Nparam",fc2_weights.shape[0])
fsl_grp.createDimension("f_Nnodes",fc2_weights.shape[1])
#fsl_grp.createDimension("f_Nparam",len(parameter_FSL))
#fsl_grp.createDimension("f_Nnodes",fsl_node/NDCONV)
fsl_grp.createDimension("f_Ximage_size",XIMAGE_SIZE)
fsl_grp.createDimension("f_Yimage_size",YIMAGE_SIZE)
fsl_grp.createDimension("f_deepness",DEEPNESS)
fsl_grp.createDimension("f_ncomp",NCOMP)
...
"""
fsl_grp.createDimension("f_Xkernel_size",KERNELSZ)
fsl_grp.createDimension("f_Ykernel_size",KERNELSZ)
fsl_grp.createDimension("f_Ndin",fsl_N_in)
fsl_grp.createDimension("f_Ndout",fsl_N_out)
"""

#Create variables FSL
fsl_fc2w = fsl_grp.createVariable('fc2_weights', 'f4', ('f_Nparam','f_Nnodes'))
fsl_fc2b = fsl_grp.createVariable('fc2_biases', 'f4', ('f_Nnodes',))



"""
fsl_dconv_w_0 = fsl_grp.createVariable('dconv_weights_0', 'f4', ('f_Xkernel_size','f_Ykernel_size','f_deepness','f_Nparam'))
#fsl_dconv_w_1 = fsl_grp.createVariable('dconv_weights_1', 'f4', ('f_Xkernel_size','f_Ykernel_size','f_Ndin','f_Ndout'))
#fsl_dconv_b_0 = fsl_grp.createVariable('dconv_biases_0', 'f4',('f_Xkernel_size',))
fsl_dconv_w_1 = fsl_grp.createVariable('dconv_weights_1', 'f4', ('f_Xkernel_size','f_Ndin','f_Ndout'))
fsl_dconv_b_0 = fsl_grp.createVariable('dconv_biases_0', 'f4',('f_Xkernel_size',))
fsl_dconv_b_1 = fsl_grp.createVariable('dconv_biases_1', 'f4',('f_Ndin',))
"""
...
for i in range(NDCONV):
    ...
    #fsl_grp.createVariable('dconv_weights_'+str(i), 'f4', ('f_Xkernel_size','f_Ykernel_size','f_Ndin','f_Ndout'))
    #fsl_grp.createVariable('dconv_biases_'+str(i), 'f4',('f_Ndin',))

## Load data for FSL 
#Load fc weight and biases
...
"""
#Ecrit en dur for debug - a reecrire + add second layer
w,b=sess.run([dconv_weights[0],dconv_biases[0]])
fsl_grp.variables["dconv_weights_0"][:,:,:,:]=w[0]
fsl_grp.variables["dconv_biases_0"][:,]=b[0]

w,b=sess.run([dconv_weights[1],dconv_biases[1]])
fsl_grp.variables["dconv_weights_1"][:,:,:]=w[1]
fsl_grp.variables["dconv_biases_1"][:,]=b[0]
"""


#PLOP
fw,fb,afsl,apar=sess.run([fc2_weights,fc2_biases,ampfsl,params])
np.save('/export/home/tfoulquier/heal_new/FSLC2W.npy',fw)
np.save('/export/home/tfoulquier/heal_new/FSLC2B.npy',fb)
np.save('/export/home/tfoulquier/heal_new/FSLAMP.npy',afsl)
np.save('/export/home/tfoulquier/heal_new/FSLPAR.npy',apar)
for i in range(NDCONV):
    w,b=sess.run([dconv_weights[i],dconv_biases[i]])
    np.save('/export/home/tfoulquier/heal_new/FSL_ww_%d_pol.npy'%(i),w)
    np.save('/export/home/tfoulquier/heal_new/FSL_bbs_%d_pol.npy'%(i),b)
np.save('/export/home/tfoulquier/heal_new/FSLDATA.npy',idata)
np.save('/export/home/tfoulquier/heal_new/FSLCORR.npy',thecorr)
...
INFO[2]=YIMAGE_SIZE
INFO[3]=NDCONV
INFO[4]=float(DOFULLYCONNECTED==True)
INFO[5]=DEEPNESS
INFO[6]=NCOMP
INFO[7]=SCALE
INFO[8]=KERNELSZ
...
INFO[10]=KERNELSZ
INFO[11]=corrnorm
#np.save('/export/home/jmdeloui/heal_new/CNN_FSL_NET_INFO.npy',INFO)
np.save('/export/home/tfoulquier/CNN_FSL_NET_INFO.npy',INFO)
