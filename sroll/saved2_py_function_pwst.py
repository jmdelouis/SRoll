...
def init_shape_from_files(TFLEARN,bolotab,rank,CNN_LEARN_MODEL,tmpname):
    ...
    for elt in CNN_names:
      ...
      if(elt=='FSL'):
        print(netcdf[elt].variables['fc2_weights'])
...
# ---------------------------------------------------------------------------------------- 
"""
def old_init_shape_from_files(TFLEARN,bolotab,rank,CNN_LEARN_MODEL,tmpname):
    ...
    for elt in CNN_names:
      ...
      if NFC[elt]==False:
        if LEARNFL[elt] == False:
          ...
          mynetwork[elt]['fc2_weights'] = tf.Variable(np.load(path_npy+"/%sC2W.npy"%(elt)),name='fc2_weights_%s'%(elt))
          mynetwork[elt]['fc2_biases'] = tf.Variable(np.load(path_npy+"/%sC2B.npy"%(elt)),name='fc2_biases_%s'%(elt))
      ...
      for i in range(NDCONV):
        if LEARNCONV[elt] == False:
          mynetwork[elt]['dconv_weights'][i]=tf.constant(np.load(path_npy+'/%s_ww_%d_pol.npy'%(elt,i)))
          mynetwork[elt]['dconv_biases'][i]=tf.constant(np.load(path_npy+'/%s_bbs_%d_pol.npy'%(elt,i)))
        else:
          mynetwork[elt]['dconv_weights'][i]=tf.Variable(np.load(path_npy+'/%s_ww_%d_pol.npy'%(elt,i)),name='dconv_weights_%s_%02d'%(elt,i))
          mynetwork[elt]['dconv_biases'][i]=tf.Variable(np.load(path_npy+'/%s_bbs_%d_pol.npy'%(elt,i)),name='dconv_biases_%s_%02d'%(elt,i))         
