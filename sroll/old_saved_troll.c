...
{ 
  ...
  if (out_itt==0) {
    if (python_rank==0) {
      arglist = Py_BuildValue("(s[s]lls)", Param->CNN_WEIGHTS,pixnames[0],(long)rank,(long) Param->CNN_LEARN_PARAM,Param->CNN_TMPID);
      if (nbolo==4)
	arglist = Py_BuildValue("(s[s,s,s,s]lls)", Param->CNN_WEIGHTS,pixnames[0],pixnames[1],
				pixnames[2],pixnames[3],(long)rank,(long) Param->CNN_LEARN_PARAM,Param->CNN_TMPID);
      if (nbolo==8)
	arglist = Py_BuildValue("(s[s,s,s,s,s,s,s,s]lls)", Param->CNN_WEIGHTS,pixnames[0],pixnames[1],
				...
				rank,(long) Param->CNN_LEARN_PARAM,Param->CNN_TMPID);
      if (nbolo==11)
  ...
  if (python_rank==0) {
	...
	if (rank==0) {
	  fprintf(stderr,"Itt %d loss=%10.4g Dloss=%10.4g Lr=%.4f Amp=%.4g Dt=%.4f\n",(int) itt,sqrt(l_loss[0]/l_loss[1]),deltaloss,loss[2],loss[3],dt);
	}
