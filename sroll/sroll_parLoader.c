...
PyObject * load_PyParams(char* path){
    ...
    if ((pDict = PyModule_GetDict(pModule)) == NULL) {
      fprintf(stderr,"Error: PyModule_GetDict\n");
      PyErr_Print();
      return -1;
    }

    // pFunc is also a borrowed reference 
    if ((pFunc = PyDict_GetItemString(pDict, "main")) == NULL) {
      fprintf(stderr,"Error: PyModule_GetItemString\n");
      PyErr_Print();
      return -1;
    }
...
int sroll_readParam(sroll_parContent *param, const char *filename) {
      ...
      if(sroll_paramDef_list[i].isList){    
        int ListSize = getListSize(pyParams,name);
          ...
          currentListValues = malloc(ListSize * sizeof(PIOSTRING));
            ...
            sroll_updateParam(param,name,currentListValues,currentListIndex);
          }
      }else{
