...
def init_calcul_pwst(nout):
    ...
    #PARAM    
    path = '/export/home/jmdeloui/heal_cnn/WIDXR_%d.npy'    
...
def init_shape_from_files(TFLEARN,bolotab,rank,tmpid,tmpname):
    ...
    path_npy ="/export/home/jmdeloui/heal_cnn/"
    ...
    for elt in CNN_names:
      #Load file
      INFO=np.load('/export/home/jmdeloui/TROLL/SrollEx/run_troll/CNN_%s_NET_INFO.npy'%(elt))
      ...
      ysize=int(INFO[2])
      NDCONV=int(INFO[3])
      DOFULLYCONNECTED=True#int(INFO[4])==1
     
      if elt == 'MAP':
        DOFULLYCONNECTED=False
      #netinfo labels initialisation
      #tab_name = []
      #tab_name.append(elt)
      #for i in range(0,len(bolotab)):
      #	tab_name.append(bolotab[i])
      #print("DEBUG test =>"+str(tab_name))

      mynetwork[elt]['FromFile']=True

      if DOFULLYCONNECTED==True:
        if elt == "MAP":
          mynetwork[elt]['fc2_weights'] = tf.constant(np.load(path_npy+"FSFC2W.npy"))
          mynetwork[elt]['fc2_biases'] = tf.constant(np.load(path_npy+"FSFC2B.npy"))
        else:
          mynetwork[elt]['fc2_weights'] = tf.Variable(np.load(path_npy+"FSFC2W.npy"))
          mynetwork[elt]['fc2_biases'] = tf.Variable(np.load(path_npy+"FSFC2B.npy"))

      mynetwork[elt]['dconv_weights']= {}
      mynetwork[elt]['dconv_biases']= {}

      for i in range(NDCONV):
        #mynetwork[elt]['lw%d'%(i+ib*NDCONV)]=np.load(path_npy+'%s_%s_NET_w_%d.npy'%(tmpname,bolotab[ib],i))
        #mynetwork[elt]['lb%d'%(i+ib*NDCONV)]=np.load(path_npy+'%s%s_%s_NET_b_%d.npy'%(tmpname,bolotab[ib],i))
        #mynetwork[elt]['dconv_weights'][i]=tf.constant(np.load(path_npy+'fsl_ww_%d_pol.npy'%(i)))
        #mynetwork[elt]['dconv_biases'][i]=tf.constant(np.load(path_npy+'fsl_bbs_%d_pol.npy'%(i)))

        if elt == "MAP":
          mynetwork[elt]['dconv_weights'][i]=tf.Variable(np.load('/export/home/jmdeloui/heal_cnn/MAPCNN_W%d.npy'%(i)))
          mynetwork[elt]['dconv_biases'][i]=tf.Variable(np.load('/export/home/jmdeloui/heal_cnn/MAPCNN_B%d.npy'%(i)))
        else:
          mynetwork[elt]['dconv_weights'][i]=tf.Variable(np.load(path_npy+'fsl_ww_%d_pol.npy'%(i)))
          mynetwork[elt]['dconv_biases'][i]=tf.Variable(np.load(path_npy+'fsl_bbs_%d_pol.npy'%(i)))         
...
def init_shape_from_file(TFLEARN,bolotab,rank,tmpid,tmpname):
  ...
  #path_npy ="/scratch/cnt0028/ias1717/SHARED/bware/sroll3_theo/extraball/SrollEx/run_troll/npy_files" 
...
def init_network(mynetwork,signal,weights,TCO1,TSI1,MAT0,MAT1,MAT2,hidx,idx,realpix): #,in_param,in_convw,in_convb,in_flw,in_flb):
    ...
    mapq=np.load('/export/home/tfoulquier/data_sroll/MAP/DUSTMODEL_857_Q.npy').astype('float32')
    mapu=np.load('/export/home/tfoulquier/data_sroll/MAP/DUSTMODEL_857_U.npy').astype('float32')
    ...
    for elt in netinfo:
      ...
      npar+=mynetwork[elt]['BATCHSZ']*mynetwork[elt]['NTOTHIDDEN']
        ...
        #if nonet:
        #   params[elt] = tf.Variable(0.1*baserandom[np.arange(netinfo[elt]['BATCHSZ']*netinfo[elt]['XIMAGE_SIZE']*netinfo[elt]['YIMAGE_SIZE'],dtype='int')%MAXRAND].reshape([netinfo[elt]['BATCHSZ']*netinfo[elt]['XIMAGE_SIZE']*netinfo[elt]['YIMAGE_SIZE']]))

        #else:
        #    params[elt] = tf.Variable(0.1*baserandom[np.arange(netinfo[elt]['BATCHSZ']*netinfo[elt]['NTOHIDDEN'],dtype='int')%MAXRAND].reshape(netinfo[elt]['BATCHSZ'],netinfo[elt]['NTOTHIDDEN']))
    ...
    map= tf.reshape(model(params["MAP"],netinfo["MAP"],mynetwork["MAP"]["rank"]),[netinfo["MAP"]['XIMAGE_SIZE']*2])
    
    ampmap=tf.Variable(np.array([1.0],dtype='float32'))
    ...
    ampfsl=tf.constant(np.load('/export/home/jmdeloui/heal_cnn/FSAMP.npy')) #CHANGE path
    corrfsl =tf.reshape(ampfsl * tf.transpose(model(params["FSL"],netinfo["FSL"],mynetwork["FSL"]['rank'])),[netinfo["FSL"]['BATCHSZ']*netinfo["FSL"]['XIMAGE_SIZE']*netinfo["FSL"]['YIMAGE_SIZE']])
   

    toifsl= tf.gather(corrfsl,bidx)
    ...
    pred = tf_CO*vsignal_q + tf_SI*vsignal_u+deltag
...
# ---------------------------------------------------------------------------------------- 
def get_correction(myrun,num,str_id_value):
  sess=myrun['sess']
 
  
  print("PLOP " + str(str_id_value))

  #/ccc/work/cont003/gen11364/foulquit/sroll_data/SIMU/857-1_APR20_stimSHPR_000.float32.bin
