...
long nadufit[100]; // 100 max number of bolometers
...
double rg_vals[4*32000];
int rg_start[32000];
int rg_end[32000];

double *xspladu;
BSpline *bspline[100]; // 100 max number of bolometers
BSpline *bsplineTime = NULL;
...
void minimize_gain_gi(double *ix2,double *gaingi)
{
    ...
    if (ndata>0&&flgpix[k]>0) {
      ...
      for (l1=0;l1<ndata;l1++) {
        ...
        if (flg_rg[htmp[l1].ib][ri1]!=0) {
          ...
          if (Param->REMHDIP==0) {
            SI+=htmp[l1].w*(htmp[l1].sig*g1-htmp[l1].fsl-htmp[l1].dip-htmp[l1].corr_nl);
            SQ+=htmp[l1].w*(htmp[l1].sig*g1-htmp[l1].fsl-htmp[l1].dip-htmp[l1].corr_nl)*CO1;
            SU+=htmp[l1].w*(htmp[l1].sig*g1-htmp[l1].fsl-htmp[l1].dip-htmp[l1].corr_nl)*SI1;
          }
          else {
            SI+=htmp[l1].w*(htmp[l1].sig*g1-htmp[l1].fsl-htmp[l1].freefree-htmp[l1].corr_nl);
            SQ+=htmp[l1].w*(htmp[l1].sig*g1-htmp[l1].fsl-htmp[l1].freefree-htmp[l1].corr_nl)*CO1;
            SU+=htmp[l1].w*(htmp[l1].sig*g1-htmp[l1].fsl-htmp[l1].freefree-htmp[l1].corr_nl)*SI1;
          }
      ...
      for (l1=0;l1<ndata;l1++) {
        ...
        if (flg_rg[htmp[l1].ib][ri1]!=0) {
          ...
          double tmp;
          if (REMHDIP==0) {
            tmp=((htmp[l1].sig*g1-htmp[l1].fsl-htmp[l1].dip-htmp[l1].corr_nl)-(SI+CO1*SQ+SI1*SU));
          }
          else {
            tmp=((htmp[l1].sig*g1-htmp[l1].fsl-htmp[l1].freefree-htmp[l1].corr_nl)-(SI+CO1*SQ+SI1*SU));
          }
  ...
  if (rank==0) {
    ...
    if (nmatco>0) {
      soff=0;
      if ((singleFreq == 0) && (Param->AVGCO143 == 1)) {
        for (i=0;i<nbolo;i++) if (freqs[i] == 143)
        ...
        for (i=0;i<nbolo;i++) if (freqs[i] == 143)
            ...
            hit2[newnr[nbolo]+nbolo*(npixbeam+GAINSTEP2)] * Param->AVGCO143;
      }
      else {
    ...
    if (nmatdust>0) {
      soff=0;
      if ((singleFreq == 0) && (Param->AVGDUST100 == 1)) {
        for (i=0;i<nbolo;i++) if (freqs[i] == 100) 
    ...
    if (flgpix[k]>0) {
      ...
      if (Param->OUT_NOPOL[0]%2==0) {
		...
		for (iii=htmp[l1].istart;iii<=htmp[l1].iend;iii++) {
		  for (jjj=rg_start[htmp[l1].rg];jjj<=rg_end[htmp[l1].rg];jjj++) {
		    int l_idx=jjj+ADURGSTEP*iii;
		      vec[htmp[l1].ib][l_idx]+=residu*htmp[l1].vspline[iii-htmp[l1].istart]
			*rg_vals[jjj-rg_start[htmp[l1].rg]+4*htmp[l1].rg]*htmp[l1].w;
		  }
		}	
	      }
	      else {
		...
		for (iii=htmp[l1].istart;iii<=htmp[l1].iend;iii++) {
		  for (jjj=rg_start[htmp[l1].rg];jjj<=rg_end[htmp[l1].rg];jjj++) {
		    ...
		    mat1[htmp[l1].ib][l_idx+nadufit[htmp[l1].ib]*(nadufit[htmp[l1].ib]+1)]+=
		      htmp[l1].vspline[iii-htmp[l1].istart]*
		      rg_vals[jjj-rg_start[htmp[l1].rg]+4*htmp[l1].rg]*htmp[l1].adu/1000.*htmp[l1].w;
		    ...
		    for (iii2=htmp[l1].istart;iii2<=htmp[l1].iend;iii2++) {
		      for (jjj2=rg_start[htmp[l1].rg];jjj2<=rg_end[htmp[l1].rg];jjj2++) {
			int l_idx2=jjj2+ADURGSTEP*iii2;
			
			mat1[htmp[l1].ib][l_idx+l_idx2*(nadufit[htmp[l1].ib]+1)]+=
			  htmp[l1].vspline[iii-htmp[l1].istart]
			  *rg_vals[jjj-rg_start[htmp[l1].rg]+4*htmp[l1].rg]
			  *htmp[l1].vspline[iii2-htmp[l1].istart]
			  *rg_vals[jjj2-rg_start[htmp[l1].rg]+4*htmp[l1].rg]*htmp[l1].w;
		      }
		    }
		  }
		}
	...
	if (ADURGSTEP>2) {
	  for (iii=htmp[l1].istart;iii<=htmp[l1].iend;iii++) {
		...
		htmp[l1].corr_nl+=htmp[l1].vspline[iii-htmp[l1].istart]
		  *rg_vals[jjj-rg_start[htmp[l1].rg]+4*htmp[l1].rg]*vec[htmp[l1].ib][l_idx];
	    }
}
	}
	else {		
    ...
    if (ndata>0&&flgpix[k]>0) {
      ...
      for (l1=0;l1<ndata;l1++) {
        ...
        if (flg_rg[htmp[l1].ib][ri1]!=0) {
          ...
          if (REMHDIP==0) SI+=htmp[l1].w*(htmp[l1].sig*g1-htmp[l1].fsl-htmp[l1].dip-htmp[l1].corr_nl);
          else SI+=htmp[l1].w*(htmp[l1].sig*g1-htmp[l1].fsl-htmp[l1].freefree-htmp[l1].corr_nl);
...
#ifndef USEDII
          htmp[l1].lvi=htmp[l1].w*htmp[l1].model/II[k];
#endif
        }
      }
      for (l1=0;l1<ndata;l1++) {
      ...
      for (l1=0;l1<ndata;l1++) {
        ...
        if (flg_rg[htmp[l1].ib][ri1]!=0) {
          double ww=htmp[l1].wp;
          double tmp=((htmp[l1].sig*g1-htmp[l1].fsl-htmp[l1].corr_nl)-(SI));
    ...
    if (ndata>0&&flgpix[k]>0) {
      ...
      for (l1=0;l1<ndata;l1++) {
        ...
        if (flg_rg[htmp[l1].ib][ri1/CUTRG]!=0) {
          ...
          double msig=((htmp[l1].sig*g1-htmp[l1].fsl-htmp[l1].dip-htmp[l1].corr_nl)-tmp);
...
int main(int argc,char *argv[])  {
  ...
  // default save covariance matrices
  ...
  if (GAINSTEPADU>1) {
    ...
    nadu3=GAINSTEPADU;
    //Param->GAINSTEP=1;
    for (i=0;i<nbolo;i++) nadufit[i]=DODISTOR;
  }
  if (DODISTOR!=0) {
    for (i=0;i<nbolo;i++) {
    ...
    if (ADURGSTEP>2) {
      bsplineTime = bspline_alloc (3,ADURGSTEP-2);
      bspline_init_uniform (bsplineTime, 0.0, 1.0);
      if (rank==0) fprintf(stderr,"DODISTOR TIME : %d %d\n",(int) ADURGSTEP,(int) bsplineTime->Ncoefs);
    }
  }
  else { 
  ...
  {
    PIOLONG nsa  = noDMC_readObject_PIOFLOAT(Param->in_template_map_I,0,12*2048*2048,skymodelI);
    if (nsa<0) {
      fprintf(stderr, "Impossible to read in_template_map_I: %s %d\n",Param->in_template_map_I,(int) nsa);
      
      exit ( -1);
    }
    nsa  = noDMC_readObject_PIOFLOAT(Param->in_template_map_Q,0,12*2048*2048,skymodelQ);
    if (nsa<0) {
      fprintf(stderr, "Impossible to read in_template_map_Q: %s %d\n",Param->in_template_map_Q,(int) nsa);
      
      exit ( -1);
    }
    nsa  = noDMC_readObject_PIOFLOAT(Param->in_template_map_U,0,12*2048*2048,skymodelU);
    if (nsa<0) {
      fprintf(stderr, "Impossible to read in_template_map_U: %s %d\n",Param->in_template_map_U,(int) nsa);
  ...
  if (Param->flag_Theo_13CO==_PAR_TRUE) {
    comap13I = (PIOFLOAT *) malloc(sizeof(PIOFLOAT)*12*128*128);
    PIOLONG nsa  = noDMC_readObject_PIOFLOAT(Param->Theo_13CO,0,12*128*128,comap13I);
    if (nsa<0) {
  ...
  if (Param->flag_in_polar_fit_Q==_PAR_TRUE) {
    inpolarQ = (PIOFLOAT *) malloc(sizeof(PIOFLOAT)*12*128*128);
    PIOLONG nsa  = noDMC_readObject_PIOFLOAT(Param->in_polar_fit_Q,0,12*128*128,inpolarQ);
    if (nsa<0) {
      fprintf(stderr, "Impossible to read in_polar_fit_Q: %s %d\n",Param->in_polar_fit_Q,(int) nsa);
      
      exit ( -1);
    }
    inpolarU = (PIOFLOAT *) malloc(sizeof(PIOFLOAT)*12*128*128);
    nsa  = noDMC_readObject_PIOFLOAT(Param->in_polar_fit_U,0,12*128*128,inpolarU);
    if (nsa<0) {
      fprintf(stderr, "Impossible to read in_polar_fit_U: %s %d\n",Param->in_polar_fit_U,(int) nsa);
  ...
  xadu = (double *) malloc(128*sizeof(double)*nbolo);
  ...
  for (ib=0;ib<nbolo;ib++) {
    ...
    PIOSTRING copsb;
    getCoPSB( copsb, pixnames[ib]);

    fprintf(stderr,"Reading data for %s, (copsb %s)\n", pixnames[ib], copsb);
    ...
    for (rg=globalRankInfo.BeginRing[rank];rg<=globalRankInfo.EndRing[rank];rg+=step) {
      ...
      if (badring[rg-globalRankInfo.BeginRing[rank]]==0) {
        ...
        for (i=0;i<npixbeam;i++) {
          if (strncmp( pixnames[ib], "353", 3) == 0) {
            // check psb or co-psb for 353PSB
            assert( strstr( Param->Theo_noPS[i*nbolo+ib], copsb) != NULL);
          }
          else {
        ...
        if (Param->flag_delta_psi == _PAR_TRUE) {
          float delta_psi_radians = Param->delta_psi * M_PI / 180.0;
          for (i=0; i<RINGSIZE; i++) {
...
//                k+begpix[rank], II, IQ, IU, QQ, QU, UU);
 ...
 //========================================================================
...
#if 1
            if (DODISTOR!=0) histo_adu[htmp[l1].adu+htmp[l1].ib*nadustep]+=ww;
#else
            if (DODISTOR!=0) histo_adu[htmp[l1].adu+htmp[l1].ib*nadustep]=1;
#endif
  ...
  if (Param->flag_ADU==_PAR_TRUE) {
	...
	PIOSTRING saveg;
        ...
        if (nadustep<32000) {
          ...
          l_histo = (double *) malloc((nadustep)*sizeof(double));
        }
        for (rrk=1;rrk<mpi_size;rrk++) {
          if (DODISTOR!=0) {
	  ...
	  for (j=1;j<ADUSTEP;j++) {
	    histo_adu[j+ib*nadustep]+=histo_adu[j-1+ib*nadustep];
	  }
	  for (j=0;j<ADUSTEP;j++) {
	    histo_adu[j+ib*nadustep]*=128./histo_adu[ADUSTEP-1+ib*nadustep];
	    if (histo_adu[j+ib*nadustep]>=128.0) histo_adu[j+ib*nadustep]=128.0; // cas pbs d'arrondi
	  }
            
	  double *nxadu=(double *) malloc(128*sizeof(double));
	  memset(nxadu,0,128*sizeof(double));
	  for (j=1;j<ADUSTEP;j++) {
	    int xii=(int) histo_adu[j+ib*nadustep];
	    if (xii>=128) xii=127;
	    xadu[xii+ib*(128)]+=j;
	    nxadu[xii]+=1;
	  }
	  for (j=0;j<128;j++) {
	    xadu[j+ib*(128)]/=nxadu[j];
	  }
	  free(nxadu);
          
          sprintf(saveg,"%s_HISTO_ADU",Param->Out_Offset[ib]);
          fprintf(stderr,"Write HISTO_ADU  %lld\n",(long long) (PIOWriteVECT(saveg,histo_adu+ib*nadustep,0,
                                                                             sizeof(PIODOUBLE)*nadustep)/sizeof(PIODOUBLE)));
          
          sprintf(saveg,"%s_XADU",Param->Out_Offset[ib]);
          fprintf(stderr,"Write XADU  %lld\n",(long long) (PIOWriteVECT(saveg,xadu+ib*(128),0,
	...
	tmpi=0;
        ...
        if (DODISTOR!=0) MPI_Send(histo_adu+ib*nadustep, sizeof(double)*nadustep, MPI_BYTE, 0, 51, MPI_COMM_WORLD);
    ...
    if (DODISTOR!=0) MPI_Bcast(histo_adu,sizeof(double)*nadustep*nbolo, MPI_BYTE, 0, MPI_COMM_WORLD);
  ...
  if (DODISTOR!=0) {
	...
	double sadu=(i-globalBeginRing)/((float)((globalEndRing+1) - globalBeginRing));
        bspline_value(bsplineTime,sadu,rg_start+i,rg_end+i);
        double *vals = bsplineTime->vals;
	for (j=rg_start[i];j<=rg_end[i];j++) rg_vals[j-rg_start[i]+4*i]=vals[j];
      }
    }
    ...
    for (k=0;k<nnbpix;k++) {
      ...
      for (l1=0;l1<ndata;l1++) {
        ...
        double xxadu=histo_adu[htmp[l1].adu+htmp[l1].ib*nadustep];
        bspline_value(bspline[htmp[l1].ib], xxadu, &(htmp[l1].istart),&(htmp[l1].iend));
        if (htmp[l1].istart<0) {
          fprintf(stderr,"xadu1=%lg %lg\n",xxadu,(double) histo_adu[htmp[l1].adu+htmp[l1].ib*nadustep]);
          exit(-1);
        }
...
#define STEPRG_HISTOADU (128)
    ...
    if (ADURGSTEP>2) {
      double **spline_stat = (double **) malloc(nbolo*sizeof(double *));
      for (i=0;i<nbolo;i++) {
	spline_stat[i] = (double *) malloc(sizeof(double)*STEPRG_HISTOADU*3200);
	memset(spline_stat[i],0,STEPRG_HISTOADU*3200*sizeof(double));
      }
	...
	int l1;
	for (l1=0;l1<ndata;l1++) {
	  spline_stat[htmp[l1].ib][(int)(htmp[l1].adu/10)+3200*((int) (htmp[l1].sadu*STEPRG_HISTOADU))]+=1;
	}
      }
	...
	if (rank==0) {
	  ...
	  for (rrk=1;rrk<mpi_size;rrk++) {
	    ...
	    for (j=0;j<3200*STEPRG_HISTOADU;j++) spline_stat[i][j]+=l_spline_stat[j];
	  }
	  free(l_spline_stat);

          for (j=0;j<STEPRG_HISTOADU;j++) {
	     for (k=1;k<3200;k++) spline_stat[i][k+3200*j]+=spline_stat[i][k-1+3200*j];
             for (k=0;k<3200;k++) spline_stat[i][k+3200*j]=spline_stat[i][k+3200*j]*128/spline_stat[i][3199+3200*j];
          }	  
	  ...
	  fwrite(spline_stat[i],3200*STEPRG_HISTOADU*sizeof(double),1,fp);
	  fclose(fp);
#endif
	}
	else {
	  MPI_Send(spline_stat[i], sizeof(double)*3200*STEPRG_HISTOADU, MPI_BYTE, 0, 51, MPI_COMM_WORLD);
	}
	
	MPI_Bcast(spline_stat[i],sizeof(double)*3200*STEPRG_HISTOADU, MPI_BYTE, 0, MPI_COMM_WORLD);
      }
      ...
      for (k=0;k<nnbpix;k++) {
         ...
         for (l1=0;l1<ndata;l1++) {
           ...
           double xxadu=spline_stat[htmp[l1].ib][(int)(htmp[l1].adu/10)+3200*((int) (htmp[l1].sadu*STEPRG_HISTOADU))];
           bspline_value(bspline[htmp[l1].ib], xxadu, &(htmp[l1].istart),&(htmp[l1].iend));
           if (htmp[l1].istart<0) {
              fprintf(stderr,"xadu2=%lg %lg %d %d\n",xxadu,(double) spline_stat[htmp[l1].ib][(int)(htmp[l1].adu/10)+3200*((int) (htmp[l1].sadu*STEPRG_HISTOADU))],(int)(htmp[l1].adu/10),((int) (htmp[l1].sadu*STEPRG_HISTOADU)));
              exit(-1);
           }
           if (htmp[l1].iend-htmp[l1].istart>3) fprintf(stderr,"SUP 4 %ld\n",(long) (htmp[l1].iend-htmp[l1].istart));
           double *vals = bspline[htmp[l1].ib]->vals;
      ...
      for (i=0;i<nbolo;i++) free(spline_stat[i]); 
      free(spline_stat);
      for (i=0;i<nbolo;i++) nadufit[i]=ADURGSTEP*nadufit[i];
    } // END OF ADURGSTEP>2
...
// nltemp could be removed
	...
	fit_adu_nl(x3,gain,ltemp,nltemp);
      ...
      while (surveys != 0) {
        ...
        for (k=0; k<nnbpix; k++) { // for each pixel in the rank
          ...
          for (i=0;i<ndata;i++) {
            ...
            if (flg_rg[ib][ri]!=0 && use_bolo==1) {
              ...
              double sig_corr = htmp[i].sig*gg2-htmp[i].fsl-htmp[i].corr_nl;
        ...
        for (p=0;p<3;p++) {
          ...
          if (Param->OUT_NOPOL[detset]%2==0 || p==0 || (p==1 && Param->OUT_NOPOL[detset]%2==1 && Param->OUT_NOPOL[0]%2==0)) {
            ...
            if (p==1 && Param->OUT_NOPOL[detset]%2==1 && Param->OUT_NOPOL[0]%2==0) strcpy(suffix,"C"); // produce "corrected" single bolo maps when used in a polarised detset
  ...
  for (k=0;k<nnbpix;k++) {
	...
	if (flg_rg[htmp[l1].ib][ri1]!=0) {
	  ...
	  double sig_corr = htmp[l1].sig*gg2-htmp[l1].fsl-htmp[l1].corr_nl;
	  if (REMHDIP==0) sig_corr-=htmp[l1].dip;
	  else sig_corr-=htmp[l1].freefree;
