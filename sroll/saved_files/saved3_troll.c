...
{ 
  ...
  if (Param->flag_FSLCNN==_PAR_TRUE) {
    ...
    if (rank==0) {
      fprintf(stderr,"CNN description should be done using NETINF files\n");
    }
    exit(0);
#if 0
  ...
  arglist = Py_BuildValue("(l)", (long) all_ndata);
    ...
    py_MAT0    = EXECPYTHON(PyObject_CallObject(MyPythonBackend.allocf32, arglist));
    py_MAT1    = EXECPYTHON(PyObject_CallObject(MyPythonBackend.allocf32, arglist));
    py_MAT2    = EXECPYTHON(PyObject_CallObject(MyPythonBackend.allocf32, arglist));
  ...
  PyObject *py_num  = EXECPYTHON(PyObject_CallObject(MyPythonBackend.alloci32, arglist));
  //Py_DECREF(arglist);

  float **correction=(float **) malloc(sizeof(float *)*ncorr);
  for (m=0;m<npixbeam;m++)  {
    if (DOCNN[m]!=0) {
      int *iter_docnn = (int *) (PyArray_DATA((PyArrayObject *)py_num));
      *iter_docnn = DOCNN[m]-1;
      ...
      correction[DOCNN[m]-1] = (float *) malloc(sizeof(float)*all_ndata);
      memcpy(correction[DOCNN[m]-1],PyArray_DATA((PyArrayObject *)thepred),sizeof(float)*all_ndata);
  ...
  //arglist = Py_BuildValue("(l)", (long) 1);
  //PyTuple_SetItem(arglist,0,myrun);
  //PyObject *close=EXECPYTHON(PyObject_CallObject(MyPythonBackend.close, arglist));
  //BUG IF RUN THE NEXT LINES ??? TO BE CLEANED
  //Py_DECREF(close);
  //Py_DECREF(arglist);
#if 0
  //if (Param->CNN_DO_TRANSFER_LEARNING == 0) {
  ...
  for (k=0;k<nnbpix;k++) {
	...
	//	htmp[l1].corr_cnn=l_incorr[htmp[l1].ib*CNN_YSIZE*CNN_XSIZE+iring*CNN_XSIZE+ipha];
	  //htmp[l1].corr_cnn=l_incorr[htmp[l1].ib*CNN_YSIZE*CNN_XSIZE+iring*CNN_XSIZE+ipha];
	//}
	htmp[l1].corr_cnn=correction[0][iii]; //htmp[l1].ib*CNN_YSIZE*CNN_XSIZE+iring*CNN_XSIZE+ipha];
	iii++;
      }
    }
  }
  ...
  if (NOMOREFITTED==0) {
    ...
    for (m=0;m<npixbeam;m++)  {
      if (DOCNN[m]!=0) {
	for (i=0;i<nbolo;i++) {
	  x3[newnr[nbolo]+nbolo*(GAINSTEP)+m*nbolo+i]=0;
	}
      }
    }
  }
...
#define STEPRG_HISTOADU (128)
  ...
  for (int iter = 0; iter < number_of_iterations; iter++) {
	...
	for (j=0;j<GAINSTEP;j++) {
      ...
      if (COMP_CNN>0&&itt>=5) { //itt>=2&&itt<Param->NITT-2) {
	//fit_cnn2d_rg(x3,gain);
	fit_cnn(x3,gain,itt-5);
      }
