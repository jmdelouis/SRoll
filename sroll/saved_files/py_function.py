...
def alloc_convw(mynetwork):
  ...
  for ib in range(mynetwork['nbolo']):
    for i in range(mynetwork['NDCONV']):
      outobj["%03d_%d"%(i,ib)] = np.zeros([mynetwork['KERNELSZ'] *mynetwork['KERNELSZ'] *mynetwork['NUM_DCONV_CHAN'][i+1]*mynetwork['NUM_DCONV_CHAN'][i]],dtype='float32')
  return(outobj)
...
def alloc_convb(mynetwork):
  ...
  for ib in range(mynetwork['nbolo']):
    for i in range(mynetwork['NDCONV']):
      outobj["%03d_%d"%(i,ib)] = np.zeros([mynetwork['NUM_DCONV_CHAN'][i+1]],dtype='float32')
  return(outobj)
...
def get_convw(myrun):
  ...
  for ib in range(mynetwork['nbolo']):
    for i in range(mynetwork['NDCONV']):
      outobj["%03d_%d"%(i,ib)] = sess.run([myrun['dconvw']])[0]
  return(outobj)
...
def get_convb(myrun):
  ...
  for ib in range(mynetwork['nbolo']):
    for i in range(mynetwork['NDCONV']):
      outobj["%03d_%d"%(i,ib)] = sess.run([myrun['dconvb']])[0]
  return(outobj)
...
def calc_grad(myrun):
    ...
    for i in range(myrun['ngradient']):
      ...
      #if myrun['rank']==0:
      #sys.stderr.write(' GRAD [%10.3g,%10.3g,%10.3g] VAL [%10.6g,%10.6g,%10.6g] \n'%(vvv[0][0,0,0],vvv[0][0,1,0],vvv[0][0,2,0],vvv[1][0,0,0],vvv[1][0,1,0],vvv[1][0,2,0]))
...
def calc_opti(myrun):
    ...
    if myrun['FromFile']==False:
      if myrun['ISFL']==True:
        for ib in range(myrun['nbolo']):
          vw,vb=sess.run([myrun['flw'],myrun['flb']])
          vgrad["%03d"%(1+ib)] = np.array(vw)
          vgrad["%03d"%(1+myrun['nbolo']+ib)] = np.array(vb)
          nnvar=1+2*myrun['nbolo']
        else:
          nnvar=1
      ...
      for ib in range(myrun['nbolo']):
        for i in range(ndconv):
          ...
          vgrad["%03d"%(nnvar+i*2*myrun['nbolo']+ib)] = np.array(vw)
          vgrad["%03d"%(nnvar+(i*2+1)*myrun['nbolo']+ib)] = np.array(vb)
    return(vgrad)
