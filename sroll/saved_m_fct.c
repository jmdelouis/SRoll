void minimize_gain_nopol_tf(double *ix2,double *gaingi){
  ...
  //gaingi -> gain de chaque detecteur entrer
    ...
    if (ndata>0&&flgpix[k]>0) {
      ...
      for (l1=0;l1<ndata;l1++) {
        ...
        if (flg_rg[htmp[l1].ib][ri1]!=0) {
          htmp[l1].wp=0;
	        //init cacul signal
          if (REMHDIP==0) SI+=htmp[l1].w*(htmp[l1].sig*g1-htmp[l1].fsl-htmp[l1].dip-htmp[l1].corr_nl-htmp[l1].corr_cnn); // Calcul du signal 
          else SI+=htmp[l1].w*(htmp[l1].sig*g1-htmp[l1].fsl-htmp[l1].freefree-htmp[l1].corr_nl-htmp[l1].corr_cnn);
 

          
          #ifdef USEDII
          dii[htmp[l1].gi+htmp[l1].ib*GAINSTEP ]+=htmp[l1].w*htmp[l1].dip; //dii 
          #endif
      ...
      for (l1=0;l1<ndata;l1++) {
        ...
        if (flg_rg[htmp[l1].ib][ri1]!=0) {
          ...
          #ifndef USEDII
          htmp[l1].lvi=htmp[l1].w*htmp[l1].dip/II[k]; //R_ij 
          #endif
        }
      }
      ...
      //Proj_data -> return b2 and hitcount -> hit2 ******************************************************
      for (l1=0;l1<ndata;l1++) {
        long ri1=htmp[l1].rg-globalBeginRing; //test validite pixel 
  ...
  if (rank==0) {
    ...
    double soff=0;
    for (i=0;i<newnr[nbolo];i++) soff+=hit2[0]*ix2[i]; //val proj_grad  //newnr = 
    ...
    if (Param->flag_AVGR0==_PAR_TRUE) {
      soff=0;
      for (i=0;i<nbolo;i++) soff+=hit2[newnr[nbolo]+nbolo*(GAINSTEP2)]*ix2[newnr[nbolo]+nbolo*(GAINSTEP2)+i];
      for (i=0;i<nbolo;i++) q2[newnr[nbolo]+nbolo*(GAINSTEP2)+i]=soff;
      for (i=0;i<nbolo;i++) b2[newnr[nbolo]+nbolo*(GAINSTEP2)+i]+=hit2[newnr[nbolo]+nbolo*(GAINSTEP2)]*Param->AVGR0;
    }
    ...
    if (nmatco>0) {
      ...
      if ((singleFreq == 0) && (Param->flag_AVG12CO == _PAR_TRUE)) {
        for (i=0;i<nbolo;i++) if (freqs[i] == Param->AVFCO)
            soff += hit2[newnr[nbolo]+nbolo*(npixbeam+GAINSTEP2)]* ix2[newnr[nbolo]+nbolo*(npixbeam+GAINSTEP2)+i];
        for (i=0;i<nbolo;i++) if (freqs[i] == Param->AVFCO)
            q2[newnr[nbolo]+nbolo*(npixbeam+GAINSTEP2)+i] = soff;
        for (i=0;i<nbolo;i++) if (freqs[i] == Param->AVFCO)
            b2[newnr[nbolo]+nbolo*(npixbeam+GAINSTEP2)+i] +=hit2[newnr[nbolo]+nbolo*(npixbeam+GAINSTEP2)] * Param->AVG12CO;
      }
      else {
        for (i=0;i<nbolo;i++) soff+=hit2[newnr[nbolo]+nbolo*(npixbeam+GAINSTEP2)]*ix2[newnr[nbolo]+nbolo*(npixbeam+GAINSTEP2)+i];
        for (i=0;i<nbolo;i++) q2[newnr[nbolo]+nbolo*(npixbeam+GAINSTEP2)+i]= soff;
        for (i=0;i<nbolo;i++) b2[newnr[nbolo]+nbolo*(npixbeam+GAINSTEP2)+i]+= hit2[newnr[nbolo]+nbolo*(npixbeam+GAINSTEP2)]*Param->AVG12CO;
      }
    }
    if (nmatdust>0) {
      soff=0;
      if ((singleFreq == 0) && (Param->flag_AVGDUST100 == _PAR_TRUE)) {
        for (i=0;i<nbolo;i++) if (freqs[i] == Param->AVFDUST)
            soff += hit2[newnr[nbolo]+nbolo*(npixbeam+GAINSTEP2)+nmatco] * ix2[newnr[nbolo]+nbolo*(npixbeam+GAINSTEP2)+nmatco+i];
        for (i=0;i<nbolo;i++) if (freqs[i] == Param->AVFDUST)
            q2[newnr[nbolo]+nbolo*(npixbeam+GAINSTEP2)+nmatco+i] = soff;
        for (i=0;i<nbolo;i++) if (freqs[i] == Param->AVFDUST)
            b2[newnr[nbolo]+nbolo*(npixbeam+GAINSTEP2)+nmatco+i] += hit2[newnr[nbolo]+nbolo*(npixbeam+GAINSTEP2)+nmatco] * Param->AVGDUST100;
      }
      else {
        for (i=0;i<nbolo;i++) soff+=hit2[newnr[nbolo]+nbolo*(npixbeam+GAINSTEP2)+nmatco]* ix2[newnr[nbolo]+nbolo*(npixbeam+GAINSTEP2)+nmatco+i];
        for (i=0;i<nbolo;i++) q2[newnr[nbolo]+nbolo*(npixbeam+GAINSTEP2)+nmatco+i]=soff;
        for (i=0;i<nbolo;i++) b2[newnr[nbolo]+nbolo*(npixbeam+GAINSTEP2)+nmatco+i]+= hit2[newnr[nbolo]+nbolo*(npixbeam+GAINSTEP2)+nmatco]*Param->AVGDUST;
      }
    }
    if (nfreefree>0) {
      soff=0;
      if ((singleFreq == 0) && (Param->flag_AVGFREEFREE == _PAR_TRUE)) {
	      for (i=0;i<nbolo;i++) if (freqs[i] == Param->AVFFF)
	        soff+=hit2[newnr[nbolo]+nbolo*(npixbeam+GAINSTEP2)+nmatco+nmatdust]*ix2[newnr[nbolo]+nbolo*(npixbeam+GAINSTEP2)+nmatco+nmatdust+i];
	      for (i=0;i<nbolo;i++) if (freqs[i] == Param->AVFFF)
				      q2[newnr[nbolo]+nbolo*(npixbeam+GAINSTEP2)+nmatco+nmatdust+i]=soff;
	      for (i=0;i<nbolo;i++) if (freqs[i] == Param->AVFFF)
				      b2[newnr[nbolo]+nbolo*(npixbeam+GAINSTEP2)+nmatco+nmatdust+i]+=hit2[newnr[nbolo]+nbolo*(npixbeam+GAINSTEP2)+nmatco+nmatdust]*Param->AVGFREEFREE;
      } else {
	      for (i=0;i<nbolo;i++) soff+=hit2[newnr[nbolo]+nbolo*(npixbeam+GAINSTEP2)+nmatco+nmatdust]*ix2[newnr[nbolo]+nbolo*(npixbeam+GAINSTEP2)+nmatco+nmatdust+i];
    ...
    if (NORMFITPOL==1) {
      ...
      if (DOFITPOLEFF==1) {
	      ...
	      soff=0;
	      for (i=0;i<nbolo;i++) soff+=hit2[newnr[nbolo]+nbolo*(GAINSTEP2+npixbeam-j)]*ix2[newnr[nbolo]+nbolo*(GAINSTEP2+npixbeam-j)+i];
  ...
  for (k=0;k<nnbpix;k++) if (flgpix[k]>0) { //proj_grad  **************************************
    ...
    if (ndata>0) {
      ...
      #ifdef USEDII
      memset(dii ,0,sizeof(double)*nbolo*GAINSTEP); //dii eq => x[Nb_ring]*t
      ...
      for (l1=0;l1<ndata;l1++) {
        ...
        if (flg_rg[htmp[l1].ib][ri1]!=0) {
          ...
          val2+=ix2[newnr[nbolo]+htmp[l1].ib*GAINSTEP+htmp[l1].gi]*htmp[l1].dip; //R_ij
      	  if (isnan(val2))  {
      ...
      for (ib=0;ib<nbolo;ib++) {
        ...
        for (j=0;j<npixbeam;j++) {
	        if (NOMOREFITTED!=DOCNN[j]||DOCNN[j]==0) {
	          q2[newnr[nbolo]+nbolo*GAINSTEP2+j*nbolo+ib]+=qri*dpixi[ib+j*nbolo+k*nbolo*npixbeam]; //q2 => Ax *******************
          }
        }
      }
    } // end if ndata<0 l 5089
    ...
    */
      ...
      */
      ...
      if(rank == 0){
        for(int m =0;m<npixbeam;m++){
          for(int b = 0;b<nbolo;b++){
            fprintf(stderr,"x[%d] = %lg\n",newnr[nbolo]+b+m*nbolo,x_tab[newnr[nbolo]+b+m*nbolo]);
          }
        }
      }
    */  
    ...
    if(rank ==0){
      for(int i =0;i<nmatres;i++){
        fprintf(stderr,"r2[%d] = %lg \t d2[%d] = %lg \t beta = %lg \t alpha = %lg \t hit2[%d] = %lg \n",i,r2[i],i,d2[i],beta,alpha,i,hit2[i]);
      }    
    }
    */
