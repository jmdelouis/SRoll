...
{ 
  ...
  if (out_itt==0) {
    ...
    int *ires=NULL;
    if (python_rank==0) arglist = PyTuple_New(9);
  ...
  if (python_rank==0) {
	...
	MPI_Allreduce(loss,l_loss,1,MPI_FLOAT,MPI_SUM,tensorflow_comm);
	l_loss[0]=loss[0];
	l_loss[1]=loss[1];
	gettimeofday(&tp2,NULL);
	double dt=(double)(tp2.tv_sec-tp1.tv_sec)+(1E-6)*(tp2.tv_usec-tp1.tv_usec);
	if (itt==0) deltaloss=fabs(l_loss[0]/mpi_tensorflow_size);
	else deltaloss=fabs(deltaloss-(l_loss[0]/mpi_tensorflow_size));
	if (rank==0) {
	  fprintf(stderr,"Itt %d loss=%10.4g Dloss=%10.4g Lr=%.4f Dt=%.4f\n",(int) itt,(l_loss[0]/mpi_tensorflow_size),deltaloss,loss[1],dt);
	}
	deltaloss=(l_loss[0]/mpi_tensorflow_size);
	gettimeofday(&tp1,NULL);
	//Py_DECREF(res);
      }
      itt++;
  ...
  if (python_rank==0) free(icorrection);
